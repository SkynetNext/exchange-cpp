...
SymbolPositionRecord::SymbolPositionRecord(int64_t uid, int32_t symbol, int32_t currency)
  : uid(uid), symbol(symbol), currency(currency) {}

SymbolPositionRecord::SymbolPositionRecord(int64_t uid, BytesIn& bytes) : uid(uid) {
...
void SymbolPositionRecord::Initialize(int64_t uid, int32_t symbol, int32_t currency) {
  this->uid = uid;
  this->symbol = symbol;
...
bool SymbolPositionRecord::IsEmpty() const {
  return direction == PositionDirection::EMPTY && pendingSellSize == 0 && pendingBuySize == 0;
}
...
void SymbolPositionRecord::PendingRelease(OrderAction orderAction, int64_t size) {
  if (orderAction == OrderAction::ASK) {
    pendingSellSize -= size;
  } else {
    pendingBuySize -= size;
...
int64_t SymbolPositionRecord::EstimateProfit(
  ...
  switch (direction) {
    ...
    case PositionDirection::LONG:
      if (lastPriceCacheRecord != nullptr && lastPriceCacheRecord->bidPrice != 0) {
    ...
    case PositionDirection::SHORT:
      if (lastPriceCacheRecord != nullptr && lastPriceCacheRecord->askPrice != INT64_MAX) {
...
int64_t
SymbolPositionRecord::CalculateRequiredMarginForFutures(const CoreSymbolSpecification& spec) const {
...
int64_t SymbolPositionRecord::CalculateRequiredMarginForOrder(const CoreSymbolSpecification& spec,
                                                              OrderAction action,
                                                              int64_t size) const {
  const int64_t specMarginBuy = spec.marginBuy;
  const int64_t specMarginSell = spec.marginSell;
...
void SymbolPositionRecord::ValidateInternalState() const {
  if (direction == PositionDirection::EMPTY && (openVolume != 0 || openPriceSum != 0)) {
    throw std::runtime_error("Invalid state: EMPTY direction but non-zero volume or price sum");
  }
  if (direction != PositionDirection::EMPTY && (openVolume <= 0 || openPriceSum <= 0)) {
...
int32_t SymbolPositionRecord::GetStateHash() const {
  ...
  result = 31 * result + static_cast<int32_t>(openPriceSum ^ (openPriceSum >> 32));
  result = 31 * result + static_cast<int32_t>(profit ^ (profit >> 32));
  result = 31 * result + static_cast<int32_t>(pendingSellSize ^ (pendingSellSize >> 32));
  result = 31 * result + static_cast<int32_t>(pendingBuySize ^ (pendingBuySize >> 32));
  return result;
}
...
std::string SymbolPositionRecord::ToString() const {
  ...
  oss << "SPR{u" << uid << " sym" << symbol << " cur" << currency << " pos"
      ...
      << " Σp=" << openPriceSum << " pnl=" << profit << " pendingS=" << pendingSellSize
