...
CoreSymbolSpecification::CoreSymbolSpecification(
    ...
    int32_t quoteCurrency, int64_t baseScaleK, int64_t quoteScaleK,
    int64_t takerFee, int64_t makerFee, int64_t marginBuy, int64_t marginSell)
    : symbolId(symbolId), type(type), baseCurrency(baseCurrency),
      quoteCurrency(quoteCurrency), baseScaleK(baseScaleK),
      quoteScaleK(quoteScaleK), takerFee(takerFee), makerFee(makerFee),
      marginBuy(marginBuy), marginSell(marginSell) {}
...
std::string CoreSymbolSpecification::ToString() const {
  ...
  oss << "CoreSymbolSpecification{symbolId=" << symbolId
      ...
      << ", quoteCurrency=" << quoteCurrency << ", baseScaleK=" << baseScaleK
      << ", quoteScaleK=" << quoteScaleK << ", takerFee=" << takerFee
