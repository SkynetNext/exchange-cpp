...
Order::Order(BytesIn& bytes)
    ...
    , action(OrderActionFromCode(bytes.ReadByte()))
    , uid(bytes.ReadLong())
...
int32_t Order::GetStateHash() const {
    ...
    result = 31 * result + static_cast<int32_t>(reserveBidPrice ^ (reserveBidPrice >> 32));
...
bool Order::operator==(const Order& other) const {
    ...
    // ignore timestamp
    return orderId == other.orderId && action == other.action && price == other.price
           && size == other.size && reserveBidPrice == other.reserveBidPrice
           && filled == other.filled && uid == other.uid;
}

std::string Order::ToString() const {
    std::ostringstream oss;
    oss << "[" << orderId << " " << (action == OrderAction::ASK ? 'A' : 'B') << price << ":" << size
        << "F" << filled << " U" << uid << "]";
    return oss.str();
}
