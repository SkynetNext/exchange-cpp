...
UserProfile::UserProfile(int64_t uid, UserStatus userStatus) : uid(uid), userStatus(userStatus) {}
...
int32_t UserProfile::GetStateHash() const {
    ...
    // adjustmentsCounter
    result = 31 * result + static_cast<int32_t>(adjustmentsCounter ^ (adjustmentsCounter >> 32));
...
std::string UserProfile::ToString() const {
    ...
    oss << "UserProfile{uid=" << uid << ", positions=" << positions.size()
        << ", accounts=" << accounts.size() << ", adjustmentsCounter=" << adjustmentsCounter
