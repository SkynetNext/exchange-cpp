...
REGISTER_REPORT_QUERY_TYPE(SingleUserReportQuery, ReportType::SINGLE_USER_REPORT);
...
std::unique_ptr<SingleUserReportResult> SingleUserReportQuery::CreateResult(
...
std::optional<std::unique_ptr<SingleUserReportResult>> SingleUserReportQuery::Process(
...
std::optional<std::unique_ptr<SingleUserReportResult>> SingleUserReportQuery::Process(
    ...
    if (userProfile != nullptr) {
        // Found user
        ankerl::unordered_dense::map<int32_t, SingleUserReportResult::Position> positions;
        ...
        for (const auto& [symbolId, pos] : userProfile->positions) {
            ...
            positions[symbolId] = SingleUserReportResult::Position(pos->currency,
                                                                   pos->direction,
                                                                   pos->openVolume,
                                                                   pos->openPriceSum,
                                                                   pos->profit,
