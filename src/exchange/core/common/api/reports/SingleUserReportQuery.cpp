  ...
  if (userProfile != nullptr) {
    ...
    for (const auto& [symbolId, pos] : userProfile->positions) {
      ...
      positions[symbolId] = SingleUserReportResult::Position(
        pos->currency, pos->direction, pos->openVolume, pos->openPriceSum, pos->profit,
        pos->pendingSellSize, pos->pendingBuySize);
    }

    return std::make_optional(
      std::unique_ptr<SingleUserReportResult>(SingleUserReportResult::CreateFromRiskEngineFound(
