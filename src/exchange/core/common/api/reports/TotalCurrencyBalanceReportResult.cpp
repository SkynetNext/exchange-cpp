...
TotalCurrencyBalanceReportResult::TotalCurrencyBalanceReportResult(BytesIn& bytes) {
    ...
    accountBalances = bytes.ReadBoolean()
    ...
    ordersBalances = bytes.ReadBoolean() ? new ankerl::unordered_dense::map<int32_t, int64_t>(
                                               utils::SerializationUtils::ReadIntLongHashMap(bytes))
                                         : nullptr;
    openInterestLong = bytes.ReadBoolean()
                           ? new ankerl::unordered_dense::map<int32_t, int64_t>(
                                 utils::SerializationUtils::ReadIntLongHashMap(bytes))
                           : nullptr;
    openInterestShort = bytes.ReadBoolean()
                            ? new ankerl::unordered_dense::map<int32_t, int64_t>(
                                  utils::SerializationUtils::ReadIntLongHashMap(bytes))
                            : nullptr;
}
...
std::unique_ptr<TotalCurrencyBalanceReportResult> TotalCurrencyBalanceReportResult::Merge(
    ...
    for (size_t i = 1; i < pieces.size(); i++) {
        ...
        result->openInterestShort =
            new ankerl::unordered_dense::map<int32_t, int64_t>(utils::SerializationUtils::MergeSum(
                result->openInterestShort, next->openInterestShort));
    }
