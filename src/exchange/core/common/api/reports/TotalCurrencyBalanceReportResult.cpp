...
TotalCurrencyBalanceReportResult* TotalCurrencyBalanceReportResult::CreateEmpty() {
  // Match Java: createEmpty()
  return new TotalCurrencyBalanceReportResult(nullptr, nullptr, nullptr, nullptr, nullptr, nullptr,
                                              nullptr);
}

TotalCurrencyBalanceReportResult* TotalCurrencyBalanceReportResult::OfOrderBalances(
  const ankerl::unordered_dense::map<int32_t, int64_t>& currencyBalance) {
  // Match Java: ofOrderBalances()
  auto* ordersBalances = new ankerl::unordered_dense::map<int32_t, int64_t>(currencyBalance);
  return new TotalCurrencyBalanceReportResult(nullptr, nullptr, nullptr, nullptr, ordersBalances,
                                              nullptr, nullptr);
}

TotalCurrencyBalanceReportResult::TotalCurrencyBalanceReportResult(BytesIn& bytes) {
  // Match Java: TotalCurrencyBalanceReportResult(final BytesIn bytesIn)
  accountBalances = bytes.ReadBoolean() ? new ankerl::unordered_dense::map<int32_t, int64_t>(
                                            utils::SerializationUtils::ReadIntLongHashMap(bytes))
                                        : nullptr;
  fees = bytes.ReadBoolean() ? new ankerl::unordered_dense::map<int32_t, int64_t>(
                                 utils::SerializationUtils::ReadIntLongHashMap(bytes))
                             : nullptr;
  adjustments = bytes.ReadBoolean() ? new ankerl::unordered_dense::map<int32_t, int64_t>(
                                        utils::SerializationUtils::ReadIntLongHashMap(bytes))
                                    : nullptr;
  suspends = bytes.ReadBoolean() ? new ankerl::unordered_dense::map<int32_t, int64_t>(
                                     utils::SerializationUtils::ReadIntLongHashMap(bytes))
                                 : nullptr;
  ordersBalances = bytes.ReadBoolean() ? new ankerl::unordered_dense::map<int32_t, int64_t>(
                                           utils::SerializationUtils::ReadIntLongHashMap(bytes))
                                       : nullptr;
  openInterestLong = bytes.ReadBoolean() ? new ankerl::unordered_dense::map<int32_t, int64_t>(
                                             utils::SerializationUtils::ReadIntLongHashMap(bytes))
                                         : nullptr;
  openInterestShort = bytes.ReadBoolean() ? new ankerl::unordered_dense::map<int32_t, int64_t>(
                                              utils::SerializationUtils::ReadIntLongHashMap(bytes))
                                          : nullptr;
}

void TotalCurrencyBalanceReportResult::WriteMarshallable(BytesOut& bytes) const {
  ...
  if (openInterestShort != nullptr) {
    utils::SerializationUtils::MarshallIntLongHashMap(*openInterestShort, bytes);
  }
}
...
TotalCurrencyBalanceReportResult::Merge(const std::vector<BytesIn*>& pieces) {
  ...
  for (size_t i = 1; i < pieces.size(); i++) {
    ...
    result->accountBalances = new ankerl::unordered_dense::map<int32_t, int64_t>(
      utils::SerializationUtils::MergeSum(result->accountBalances, next->accountBalances));
    ...
    result->adjustments = new ankerl::unordered_dense::map<int32_t, int64_t>(
      utils::SerializationUtils::MergeSum(result->adjustments, next->adjustments));
    ...
    result->ordersBalances = new ankerl::unordered_dense::map<int32_t, int64_t>(
      utils::SerializationUtils::MergeSum(result->ordersBalances, next->ordersBalances));
    result->openInterestLong = new ankerl::unordered_dense::map<int32_t, int64_t>(
      utils::SerializationUtils::MergeSum(result->openInterestLong, next->openInterestLong));
    result->openInterestShort = new ankerl::unordered_dense::map<int32_t, int64_t>(
      utils::SerializationUtils::MergeSum(result->openInterestShort, next->openInterestShort));
  }
...
TotalCurrencyBalanceReportResult::GetGlobalBalancesSum() const {
  // Match Java: getGlobalBalancesSum() - merges accountBalances,
  // ordersBalances, fees, adjustments, suspends
  return utils::SerializationUtils::MergeSum(accountBalances, ordersBalances, fees, adjustments,
                                             suspends);
}

bool TotalCurrencyBalanceReportResult::IsGlobalBalancesAllZero() const {
  // Match Java: isGlobalBalancesAllZero() - checks if all merged balances are
  // zero
