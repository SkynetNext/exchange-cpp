...
StateHashReportQuery::CreateResult(const std::vector<BytesIn*>& sections) {
...
StateHashReportQuery::Process(MatchingEngineRouterType* matchingEngine) {
...
std::optional<std::unique_ptr<StateHashReportResult>>
StateHashReportQuery::Process(RiskEngineType* riskEngine) {
  ...
  for (const auto& pair : fees) {
    feesHash ^= (std::hash<int32_t>{}(pair.first) << 4) ^ (std::hash<int64_t>{}(pair.second) << 5);
  }
  hashCodes[StateHashReportResult::CreateKey(
    moduleId, StateHashReportResult::SubmoduleType::RISK_FEES)] = static_cast<int32_t>(feesHash);
  ...
  hashCodes[StateHashReportResult::CreateKey(moduleId,
                                             StateHashReportResult::SubmoduleType::RISK_SUSPENDS)] =
