...
StateHashReportQuery::Process(RiskEngineType *riskEngine) {
  ...
  for (const auto &pair : fees) {
    feesHash ^= (std::hash<int32_t>{}(pair.first) << 4) ^
                (std::hash<int64_t>{}(pair.second) << 5);
  }
  hashCodes[StateHashReportResult::CreateKey(
      moduleId, StateHashReportResult::SubmoduleType::RISK_FEES)] =
  ...
  for (const auto &pair : suspends) {
    suspendsHash ^= (std::hash<int32_t>{}(pair.first) << 8) ^
                    (std::hash<int64_t>{}(pair.second) << 9);
  }
  hashCodes[StateHashReportResult::CreateKey(
