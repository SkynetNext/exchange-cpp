...
SingleUserReportResult::SingleUserReportResult(BytesIn& bytes) {
  ...
  if (hasAccounts) {
    ...
    accounts =
      std::make_unique<ankerl::unordered_dense::map<int32_t, int64_t>>(std::move(accountsMap));
  } else {
    accounts = nullptr;
  }
...
SingleUserReportResult* SingleUserReportResult::CreateFromRiskEngineFound(
  ...
  result->positions = std::make_unique<ankerl::unordered_dense::map<int32_t, Position>>(positions);
