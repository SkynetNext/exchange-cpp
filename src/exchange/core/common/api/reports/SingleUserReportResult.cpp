...
void SingleUserReportResult::Position::WriteMarshallable(BytesOut& bytes) const {
...
void SingleUserReportResult::WriteMarshallable(BytesOut& bytes) const {
    ...
    if (orders != nullptr) {
        ...
        utils::SerializationUtils::MarshallIntHashMap<OrderListType>(
            *orders, bytes, [](const OrderListType& symbolOrders, common::BytesOut& b) {
                // Match Java: SerializationUtils.marshallList(symbolOrders, bytes)
                utils::SerializationUtils::MarshallList<common::Order>(
                    symbolOrders, b, [](common::Order* order, common::BytesOut& bytesOut) {
...
SingleUserReportResult::SingleUserReportResult(BytesIn& bytes) {
    ...
    // positions
    if (bytes.ReadBoolean()) {
        positions = std::make_unique<ankerl::unordered_dense::map<int32_t, Position>>();
    ...
    // orders
    if (bytes.ReadBoolean()) {
        orders = std::make_unique<ankerl::unordered_dense::map<int32_t, std::vector<Order*>>>();
        int length = bytes.ReadInt();
        for (int i = 0; i < length; i++) {
            int32_t key = bytes.ReadInt();
...
std::unique_ptr<SingleUserReportResult> SingleUserReportResult::Merge(
...
SingleUserReportResult* SingleUserReportResult::CreateFromMatchingEngine(
    ...
    result->orders =
        std::make_unique<ankerl::unordered_dense::map<int32_t, std::vector<Order*>>>(orders);
...
SingleUserReportResult* SingleUserReportResult::CreateFromRiskEngineFound(
    ...
    result->accounts = std::make_unique<ankerl::unordered_dense::map<int32_t, int64_t>>(accounts);
    result->positions =
        std::make_unique<ankerl::unordered_dense::map<int32_t, Position>>(positions);
...
std::unique_ptr<SingleUserReportResult> SingleUserReportResult::CreateFromRiskEngineNotFound(
    int64_t uid) {
