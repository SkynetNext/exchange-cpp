    ...
    ExchangeCoreImpl(ExchangeCore::ResultsConsumer resultsConsumer,
        ...
        for (int32_t shardId = 0; shardId < matchingEnginesNum; shardId++) {
            matchingEngines_.push_back(
                std::make_unique<processors::MatchingEngineRouter>(shardId,
                                                                   matchingEnginesNum,
        ...
        for (int32_t shardId = 0; shardId < riskEnginesNum; shardId++) {
            riskEngines_.push_back(std::make_unique<processors::RiskEngine>(shardId,
                                                                            riskEnginesNum,
                                                                            serializationProcessor_,
                                                                            sharedPool_.get(),
                                                                            exchangeConfiguration));
        ...
        class GroupingProcessorFactory
         ...
         public:
            ...
            std::shared_ptr<disruptor::EventProcessor> createEventProcessor(
                RingBufferT& ringBuffer,
        ...
        if (serializationCfg.enableJournaling) {
            class JournalingEventHandler
             ...
             public:
                ...
                void onEvent(common::cmd::OrderCommand& cmd,
                             int64_t sequence,
        ...
        for (size_t i = 0; i < riskEngines_.size(); i++) {
            ...
            auto r1Factory = R1ProcessorFactory(handler.get(),
                                                exceptionHandler_.get(),
        ...
        if (!matchingEngineHandlers_.empty()) {
            ...
            auto registerHandlers = [&afterR1](auto& handlers) {
                if (handlers.size() == 1) {
                    ...
                    afterR1.handleEventsWith(
                        *handlers[0].get(), *handlers[1].get(), *handlers[2].get());
        ...
        for (size_t i = 0; i < riskEngines_.size(); i++) {
            ...
            auto r2Factory = R2ProcessorFactory(handler.get(),
                                                exceptionHandler_.get(),
                                                "R2_" + std::to_string(i),
                                                r2Processors_,
                                                r2ProcessorsOwned_,
