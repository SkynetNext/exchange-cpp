...
SerializationUtils::MergeSum(const ankerl::unordered_dense::map<int32_t, int64_t>* map1,
...
std::vector<uint8_t> SerializationUtils::LongsLz4ToBytes(const std::vector<int64_t>& dataArray,
  ...
  std::span<const uint8_t> compressedDataSpan(compressedData);
  const char* compressedDataStart =
    reinterpret_cast<const char*>(compressedDataSpan.subspan(sizeof(int32_t)).data());
  ...
  if (decompressedSize < 0 || decompressedSize != originalSizeBytes) {
    throw std::runtime_error(
      ...
      "originalSize="
      + std::to_string(originalSizeBytes) + ", decompressedSize=" + std::to_string(decompressedSize)
      + ", longsTransferred=" + std::to_string(longsTransferred)
      + ", availableCompressedBytes=" + std::to_string(availableCompressedBytes));
  }

  return decompressedData;
}

std::vector<int64_t> SerializationUtils::BytesToLongArrayLz4(const std::vector<uint8_t>& bytes,
                                                             int padding) {
  ...
  // Compress using LZ4
  std::span<uint8_t> compressedBufferSpan(compressedBuffer);
  const int compressedSize =
    LZ4_compress_default(reinterpret_cast<const char*>(bytes.data()),
                         reinterpret_cast<char*>(compressedBufferSpan.subspan(4).data()),
                         originalSize, maxCompressedSize);
  ...
  std::memcpy(longArray.data(), compressedBuffer.data(), compressedBuffer.size());

  return longArray;
}

std::vector<int64_t> SerializationUtils::BytesToLongArray(const std::vector<uint8_t>& bytes,
                                                          int padding) {
...
std::vector<uint8_t> SerializationUtils::LongsToBytes(const std::vector<int64_t>& longs) {
...
common::Wire SerializationUtils::LongsToWire(const std::vector<int64_t>& dataArray) {
  // Match Java: SerializationUtils.longsToWire()
  // 1:1 translation
