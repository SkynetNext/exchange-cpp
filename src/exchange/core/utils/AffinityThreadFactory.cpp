...
AffinityThreadFactory::AffinityThreadFactory(ThreadAffinityMode threadAffinityMode)
...
void AffinityThreadFactory::ExecutePinned(std::function<void()> runnable) {
  ...
  try {
    ...
    LOG_INFO("{} will be running on thread={} pinned to cpu {}", "Task", threadName,
             cpuId >= 0 ? cpuId : -1);
...
int AffinityThreadFactory::AcquireAffinityLock(int32_t threadId) {
  ...
  if (GetProcessAffinityMask(GetCurrentProcess(), &processAffinityMask, &systemAffinityMask)) {
    ...
    if (threadAffinityMode_ == ThreadAffinityMode::THREAD_AFFINITY_ENABLE_PER_PHYSICAL_CORE) {
