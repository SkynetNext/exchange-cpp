...
std::array<std::array<ProcessorMessageCounter::BatchSizeData*,
                      ProcessorMessageCounter::MAX_PROCESSORS_PER_TYPE>,
           ProcessorMessageCounter::MAX_TYPES_COUNT>
...
thread_local ProcessorMessageCounter::ThreadLocalData ProcessorMessageCounter::threadLocalData_;
...
void ProcessorMessageCounter::ThreadLocalData::FlushToGlobal() {
  ...
  for (size_t i = 0; i < count_; i++) {
    ...
    BatchSizeData* data =
      ProcessorMessageCounter::GetOrCreateProcessor(entry.type, entry.processorId);
    if (data == nullptr) {
...
ProcessorMessageCounter::GetOrCreateProcessor(ProcessorType type, int32_t processorId) {
  if (processorId < 0 || processorId >= static_cast<int32_t>(MAX_PROCESSORS_PER_TYPE)) {
...
int64_t ProcessorMessageCounter::CalculatePercentile(const std::vector<int64_t>& sorted,
                                                     double percentile) {
  if (sorted.empty()) {
    return 0;
  }
...
std::vector<int64_t> ProcessorMessageCounter::GetStatistics(ProcessorType type,
                                                            int32_t processorId) {
...
std::unordered_map<std::string, std::vector<int64_t>> ProcessorMessageCounter::GetAllStatistics() {
  ...
  for (size_t typeIdx = 0; typeIdx < static_cast<size_t>(ProcessorType::MAX_TYPES); typeIdx++) {
    ProcessorType type = static_cast<ProcessorType>(typeIdx);
    for (int32_t processorId = 0; processorId < MAX_PROCESSORS_PER_TYPE; processorId++) {
      BatchSizeData* data = processors_[typeIdx][processorId];
      if (data == nullptr) {
        continue;
      }
...
void ProcessorMessageCounter::PrintStatistics(ProcessorType type, int32_t processorId) {
  ...
  LOG_INFO("  P50: {}, P90: {}, P95: {}, P99: {}, P99.9: {}", stats[3], stats[4], stats[5],
           stats[6], stats[7]);
}
...
void ProcessorMessageCounter::PrintAllStatistics() {
  ...
  for (const auto& [name, stats] : allStats) {
    ...
    LOG_INFO("[{}] Batches: {}, Min: {}, Max: {}, P50: {}, P90: {}, P95: {}, "
             "P99: {}, P99.9: {}",
             name, stats[0], stats[1], stats[2], stats[3], stats[4], stats[5], stats[6], stats[7]);
  }
  LOG_INFO("========================================");
}

std::string ProcessorMessageCounter::GetProcessorName(ProcessorType type, int32_t processorId) {
  switch (type) {
