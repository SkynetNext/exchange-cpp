...
IArtNode<V> *ArtNode4<V>::Put(int64_t key, int level, V *value) {
  ...
  while (pos < numChildren_) {
    if (nodeIndex == keys_[pos]) {
      ...
      if (nodeLevel_ == 0) {
        ...
        IArtNode<V> *resizedNode = static_cast<IArtNode<V> *>(nodes_[pos])
                                       ->Put(key, nodeLevel_ - 8, value);
  ...
  if (numChildren_ != 4) {
    ...
    if (nodeLevel_ == 0) {
      ...
      ArtNode4<V> *newSubNode = objectsPool_->Get(
    ...
    ArtNode16<V> *node16 = objectsPool_->Get(
...
template <typename V> IArtNode<V> *ArtNode4<V>::Remove(int64_t key, int level) {
  ...
  if (nodeLevel_ == 0) {
    ...
    IArtNode<V> *resizedNode =
        static_cast<IArtNode<V> *>(node)->Remove(key, nodeLevel_ - 8);
    if (resizedNode != node) {
...
std::string ArtNode4<V>::PrintDiagram(const std::string &prefix, int level) {
  // TODO: Implement diagram printing
  return prefix + "ArtNode4{nodeKey=" + std::to_string(nodeKey_) +
         ", nodeLevel=" + std::to_string(nodeLevel_) +
         ", numChildren=" + std::to_string(numChildren_) + "}";
}
