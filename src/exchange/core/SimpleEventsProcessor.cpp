...
void SimpleEventsProcessor::SendCommandResult(common::cmd::OrderCommand* cmd, int64_t seq) {
    ...
    switch (cmd->command) {
        case common::cmd::OrderCommandType::PLACE_ORDER:
            apiCmd = new common::api::ApiPlaceOrder(cmd->price,
                                                    cmd->size,
                                                    cmd->orderId,
                                                    cmd->action,
                                                    cmd->orderType,
        ...
        case common::cmd::OrderCommandType::REDUCE_ORDER:
            apiCmd =
                new common::api::ApiReduceOrder(cmd->orderId, cmd->uid, cmd->symbol, cmd->size);
            break;
        ...
        case common::cmd::OrderCommandType::BALANCE_ADJUSTMENT:
            apiCmd = new common::api::ApiAdjustUserBalance(
                cmd->uid, cmd->symbol, cmd->price, cmd->orderId);
            break;
        ...
        case common::cmd::OrderCommandType::ORDER_BOOK_REQUEST:
            apiCmd =
                new common::api::ApiOrderBookRequest(cmd->symbol, static_cast<int32_t>(cmd->size));
            break;
...
void SimpleEventsProcessor::SendTradeEvent(common::cmd::OrderCommand* cmd) {
    ...
    while (event != nullptr) {
        if (event->eventType == common::MatcherEventType::TRADE) {
            Trade trade{event->matchedOrderId,
            ...
            rejectEvent = new RejectEvent{
                cmd->symbol, event->size, event->price, cmd->orderId, cmd->uid, cmd->timestamp};
        }
...
void SimpleEventsProcessor::SendMarketData(common::cmd::OrderCommand* cmd) {
    ...
    for (int32_t i = 0; i < marketData->askSize; i++) {
        asks.push_back(OrderBookRecord{marketData->askPrices[i],
                                       marketData->askVolumes[i],
    ...
    for (int32_t i = 0; i < marketData->bidSize; i++) {
        bids.push_back(OrderBookRecord{marketData->bidPrices[i],
                                       marketData->bidVolumes[i],
