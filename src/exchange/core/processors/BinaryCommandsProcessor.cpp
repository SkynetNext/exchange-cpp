...
class TransferRecord : public common::WriteBytesMarshallable, public common::StateHash {
 public:
    ...
    explicit TransferRecord(int expectedLength)
...
common::cmd::CommandResultCode BinaryCommandsProcessor::AcceptBinaryFrame(
    ...
    try {
        ...
        if (cmd->symbol == -1) {
            ...
            std::vector<uint8_t> decompressedBytes = utils::SerializationUtils::LongsLz4ToBytes(
                record->dataArray, record->wordsTransferred);
            ...
            if (cmd->command == common::cmd::OrderCommandType::BINARY_DATA_QUERY) {
                ...
                auto binaryCommand =
                    common::api::binary::BinaryDataCommandFactory::getInstance().createCommand(
                        commandType, bytesIn);
