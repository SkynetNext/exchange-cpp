...
bool DiskSerializationProcessor::StoreData(int64_t snapshotId,
    ...
    try {
        ...
        try {
            ...
            LOG_ERROR(
                "Can not write snapshot file: {} - WriteMarshallable failed: {}", path, ex.what());
            file.close();
            return false;
        }
...
void DiskSerializationProcessor::WriteToJournal(common::cmd::OrderCommand* cmd,
    ...
    if (dSeq + baseSeq_ == enableJournalAfterSeq_ + 1) {
        LOG_INFO(
            "Enabled journaling at seq = {} ({}+{})", enableJournalAfterSeq_ + 1, baseSeq_, dSeq);
    }
    ...
    while (is.peek() != EOF && is.good()) {
        ...
        if (cmdByte == static_cast<int8_t>(common::cmd::OrderCommandType::RESERVED_COMPRESSED)) {
            ...
            if (cmdType == common::cmd::OrderCommandType::MOVE_ORDER) {
                ...
                api->MoveOrderReplay(
                    serviceFlags, eventsGroup, timestampNs, price, orderId, symbol, uid);
                ...
                api->CancelOrderReplay(
                    serviceFlags, eventsGroup, timestampNs, orderId, symbol, uid);
                ...
                api->ReduceOrderReplay(
                    serviceFlags, eventsGroup, timestampNs, reduceSize, orderId, symbol, uid);
                ...
                api->PlaceOrderReplay(serviceFlags,
                                      eventsGroup,
                                      timestampNs,
                                      orderId,
                                      userCookie,
                ...
                api->BalanceAdjustmentReplay(serviceFlags,
                                             eventsGroup,
                                             timestampNs,
                                             uid,
                                             transactionId,
...
void DiskSerializationProcessor::FlushBufferSync(bool forceStartNextFile, int64_t timestampNs) {
    if (journalWriteBufferPos_ < static_cast<size_t>(journalBatchCompressThreshold_)) {
        ...
        const int compressedSize = LZ4_compress_default(
            journalWriteBuffer_.data(), compressed.data(), originalLength, maxCompressedSize);
