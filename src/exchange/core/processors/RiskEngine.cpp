...
void RiskEngine::HandleBinaryMessage(common::api::binary::BinaryDataCommand* message) {
    if (auto* batchAddSymbols =
            dynamic_cast<common::api::binary::BatchAddSymbolsCommand*>(message)) {
    ...
    if (spec->type == common::SymbolType::CURRENCY_EXCHANGE_PAIR) {
        ...
        if (posIt != userProfile->positions.end()) {
            ...
            position = objectsPool_->Get<common::SymbolPositionRecord>(
                ::exchange::core::collections::objpool::ObjectsPool::SYMBOL_POSITION_RECORD,
                [cmd, spec]() {
                    return new common::SymbolPositionRecord(
                        cmd->uid, spec->symbolId, spec->quoteCurrency);
                });
...
common::cmd::CommandResultCode RiskEngine::PlaceExchangeOrder(
...
common::cmd::CommandResultCode RiskEngine::AdjustBalance(
    int64_t uid,
    int32_t currency,
    int64_t amountDiff,
...
void RiskEngine::HandleMatcherEventsExchangeSell(common::MatcherTradeEvent* ev,
    ...
    if (taker != nullptr) {
        taker->accounts[quoteCurrency] += takerSizePriceForThisHandler * spec->quoteScaleK
...
void RiskEngine::HandleMatcherEventMargin(common::MatcherTradeEvent* ev,
    ...
    if (takerUp != nullptr && takerSpr != nullptr) {
        if (ev->eventType == common::MatcherEventType::TRADE) {
            ...
            int64_t sizeOpen =
                takerSpr->UpdatePositionForMarginTrade(takerAction, ev->size, ev->price);
...
int32_t RiskEngine::GetStateHash() const {
    ...
    for (const auto& pair : suspends_) {
        hash ^=
            (std::hash<int32_t>{}(pair.first) << 11) ^ (std::hash<int64_t>{}(pair.second) << 12);
    }
