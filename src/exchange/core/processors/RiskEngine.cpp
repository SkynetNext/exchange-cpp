...
RiskEngine::RiskEngine(int32_t shardId,
                       int64_t numShards,
  ...
  , exchangeId_("")
  , folder_("")
  , cfgIgnoreRiskProcessing_(false)
  , cfgMarginTradingEnabled_(false)
  , logDebug_(false) {
  // Validate numShards is power of 2
  if ((numShards & (numShards - 1)) != 0) {
    throw std::invalid_argument("Invalid number of shards " + std::to_string(numShards)
                                + " - must be power of 2");
  }
  ...
  cfgIgnoreRiskProcessing_ =
    (ordersProcCfg->riskProcessingMode
     == common::config::OrdersProcessingConfiguration::RiskProcessingMode::NO_RISK_PROCESSING);
  ...
  if (journaling::ISerializationProcessor::CanLoadFromSnapshot(
    ...
    serializationProcessor->LoadData(
      ...
      journaling::ISerializationProcessor::SerializedModuleType::RISK_ENGINE, shardId_,
      [this, sharedPool, reportsQueriesCfg](common::BytesIn* bytesIn) {
        ...
        symbolSpecificationProvider_ = std::make_unique<SymbolSpecificationProvider>(bytesIn);
        ...
        // RiskEngine
        reportQueriesHandler_ = std::make_unique<RiskEngineReportQueriesHandler>(this);
        ...
        binaryCommandsProcessor_ = std::make_unique<BinaryCommandsProcessor>(
          ...
          reportQueriesHandler_.get(), sharedPool, reportsQueriesCfg, bytesIn, shardId_);
        ...
        adjustments_ = utils::SerializationUtils::ReadIntLongHashMap(*bytesIn);
    ...
    symbolSpecificationProvider_ = std::make_unique<SymbolSpecificationProvider>();
...
bool RiskEngine::PreProcessCommand(int64_t seq, common::cmd::OrderCommand* cmd) {
  switch (cmd->command) {
    ...
    case common::cmd::OrderCommandType::BALANCE_ADJUSTMENT:
      if (UidForThisHandler(cmd->uid)) {
        auto adjustmentType =
          common::BalanceAdjustmentTypeFromCode(static_cast<int8_t>(cmd->orderType));
        cmd->resultCode =
          AdjustBalance(cmd->uid, cmd->symbol, cmd->price, cmd->orderId, adjustmentType);
      }
      return false;
    ...
    case common::cmd::OrderCommandType::BINARY_DATA_QUERY:
      ...
      if (shardId_ == 0) {
        cmd->resultCode = common::cmd::CommandResultCode::VALID_FOR_MATCHING_ENGINE;
      }
      return false;
    ...
    case common::cmd::OrderCommandType::PERSIST_STATE_RISK:
      if (serializationProcessor_ != nullptr) {
        const bool isSuccess = serializationProcessor_->StoreData(
          ...
          journaling::ISerializationProcessor::SerializedModuleType::RISK_ENGINE, shardId_, this);
...
void RiskEngine::PostProcessCommand(int64_t seq, common::cmd::OrderCommand* cmd) {
  ...
  if (marketData == nullptr
      && (mte == nullptr || mte->eventType == common::MatcherEventType::BINARY_EVENT)) {
    return;
  }

  const auto* spec = symbolSpecificationProvider_->GetSymbolSpecification(symbol);
  if (spec == nullptr) {
  ...
  if (marketData != nullptr && cfgMarginTradingEnabled_) {
    ...
    record.askPrice = (marketData->askSize != 0) ? marketData->askPrices[0] : INT64_MAX;
...
common::cmd::CommandResultCode RiskEngine::PlaceOrderRiskCheck(common::cmd::OrderCommand* cmd) {
  auto* userProfile = userProfileService_->GetUserProfile(cmd->uid);
  if (userProfile == nullptr) {
    return common::cmd::CommandResultCode::AUTH_INVALID_USER;
  }

  const auto* spec = symbolSpecificationProvider_->GetSymbolSpecification(cmd->symbol);
  if (spec == nullptr) {
...
void RiskEngine::HandleBinaryMessage(common::api::binary::BinaryDataCommand* message) {
    ...
    for (const auto& [symbolId, spec] : symbols) {
      if (spec != nullptr
        ...
        symbolSpecificationProvider_->AddSymbol(spec);
               ...
               dynamic_cast<common::api::binary::BatchAddAccountsCommand*>(message)) {
    // Handle batch add accounts
    const auto& users = batchAddAccounts->users;
    for (const auto& [uid, accounts] : users) {
      if (userProfileService_->AddUser(uid) == common::cmd::CommandResultCode::SUCCESS) {
...
RiskEngine::HandleReportQuery(common::api::reports::ReportQuery<R>* reportQuery) {
  auto result = reportQuery->Process(this);
  return result;
}

// Private helper methods (simplified implementations)
common::cmd::CommandResultCode RiskEngine::PlaceOrder(common::cmd::OrderCommand* cmd,
  ...
  if (spec->type == common::SymbolType::CURRENCY_EXCHANGE_PAIR) {
    ...
    const bool canPlaceOrder = CanPlaceMarginOrder(cmd, userProfile, spec, position);
    if (canPlaceOrder) {
...
RiskEngine::PlaceExchangeOrder(common::cmd::OrderCommand* cmd,
  ...
  const int32_t currency =
    (cmd->action == common::OrderAction::BID) ? spec->quoteCurrency : spec->baseCurrency;
  ...
  if (cmd->action == common::OrderAction::BID) {
    if (cmd->orderType == common::OrderType::FOK_BUDGET
      ...
      orderHoldAmount = utils::CoreArithmeticUtils::CalculateAmountBidTakerFeeForBudget(
    ...
    orderHoldAmount = utils::CoreArithmeticUtils::CalculateAmountAsk(cmd->size, spec);
  }
...
common::cmd::CommandResultCode
RiskEngine::AdjustBalance(int64_t uid,
                          int32_t currency,
                          int64_t amountDiff,
                          int64_t fundingTransactionId,
                          common::BalanceAdjustmentType adjustmentType) {
  const auto res =
    userProfileService_->BalanceAdjustment(uid, currency, amountDiff, fundingTransactionId);
...
void RiskEngine::HandleMatcherRejectReduceEventExchange(common::cmd::OrderCommand* cmd,
  ...
  if (takerSell) {
    ...
    if (cmd->command == common::cmd::OrderCommandType::PLACE_ORDER
      ...
      taker->accounts[spec->quoteCurrency] +=
        utils::CoreArithmeticUtils::CalculateAmountBidTakerFeeForBudget(ev->size, ev->price, spec);
    } else {
      taker->accounts[spec->quoteCurrency] +=
        utils::CoreArithmeticUtils::CalculateAmountBidTakerFee(ev->size, ev->bidderHoldPrice, spec);
    }
  }
}

void RiskEngine::HandleMatcherEventsExchangeSell(common::MatcherTradeEvent* ev,
                                                 const common::CoreSymbolSpecification* spec,
                                                 common::UserProfile* taker) {
  ...
  while (ev != nullptr) {
    ...
    if (UidForThisHandler(ev->matchedOrderUid)) {
      ...
      auto* maker = userProfileService_->GetUserProfileOrAddSuspended(ev->matchedOrderUid);
      ...
      const int64_t amountDiffToReleaseInQuoteCurrency =
        utils::CoreArithmeticUtils::CalculateAmountBidReleaseCorrMaker(size, priceDiff, spec);
  ...
  if (taker != nullptr) {
    taker->accounts[quoteCurrency] +=
      takerSizePriceForThisHandler * spec->quoteScaleK - spec->takerFee * takerSizeForThisHandler;
  }

  if (takerSizeForThisHandler != 0 || makerSizeForThisHandler != 0) {
    fees_[quoteCurrency] +=
      spec->takerFee * takerSizeForThisHandler + spec->makerFee * makerSizeForThisHandler;
  }
}

void RiskEngine::HandleMatcherEventsExchangeBuy(common::MatcherTradeEvent* ev,
                                                const common::CoreSymbolSpecification* spec,
                                                common::UserProfile* taker,
  ...
  if (taker != nullptr) {
    ...
    int64_t balanceDiff = (takerSizePriceHeldSum - takerSizePriceSum) * spec->quoteScaleK;
...
void RiskEngine::HandleMatcherEventMargin(common::MatcherTradeEvent* ev,
                                          const common::CoreSymbolSpecification* spec,
  ...
  if (takerUp != nullptr && takerSpr != nullptr) {
    if (ev->eventType == common::MatcherEventType::TRADE) {
      ...
      int64_t sizeOpen = takerSpr->UpdatePositionForMarginTrade(takerAction, ev->size, ev->price);
  ...
  if (ev->eventType == common::MatcherEventType::TRADE && UidForThisHandler(ev->matchedOrderUid)) {
    // update maker's position
    auto* maker = userProfileService_->GetUserProfileOrAddSuspended(ev->matchedOrderUid);
    auto* makerSpr = maker->GetPositionRecordOrThrowEx(spec->symbolId);
    int64_t sizeOpen =
      makerSpr->UpdatePositionForMarginTrade(OppositeAction(takerAction), ev->size, ev->price);
...
common::OrderAction RiskEngine::OppositeAction(common::OrderAction action) {
  return (action == common::OrderAction::BID) ? common::OrderAction::ASK : common::OrderAction::BID;
}

bool RiskEngine::CanPlaceMarginOrder(common::cmd::OrderCommand* cmd,
                                     common::UserProfile* userProfile,
                                     const common::CoreSymbolSpecification* spec,
  ...
  for (const auto& posPair : userProfile->positions) {
    ...
    if (recSymbol != symbol) {
      if (posPair.second->currency == spec->quoteCurrency) {
        const auto* spec2 = symbolSpecificationProvider_->GetSymbolSpecification(recSymbol);
        if (spec2 != nullptr) {
          auto lastPriceIt = lastPriceCache_.find(recSymbol);
          const LastPriceCacheRecord* lastPriceRecord =
            (lastPriceIt != lastPriceCache_.end()) ? &lastPriceIt->second : nullptr;
          ...
          freeMargin += posPair.second->EstimateProfit(*spec2, &commonLastPrice);
          freeMargin -= posPair.second->CalculateRequiredMarginForFutures(*spec2);
        }
      }
    } else {
...
int32_t RiskEngine::GetStateHash() const {
  ...
  if (symbolSpecificationProvider_ != nullptr) {
    hash ^= static_cast<std::size_t>(symbolSpecificationProvider_->GetStateHash()) << 1;
  }
  ...
  if (binaryCommandsProcessor_ != nullptr) {
    hash ^= static_cast<std::size_t>(binaryCommandsProcessor_->GetStateHash()) << 3;
  }
  ...
  for (const auto& pair : fees_) {
    hash ^= (std::hash<int32_t>{}(pair.first) << 7) ^ (std::hash<int64_t>{}(pair.second) << 8);
  }
  for (const auto& pair : adjustments_) {
    hash ^= (std::hash<int32_t>{}(pair.first) << 9) ^ (std::hash<int64_t>{}(pair.second) << 10);
  }
  for (const auto& pair : suspends_) {
    hash ^= (std::hash<int32_t>{}(pair.first) << 11) ^ (std::hash<int64_t>{}(pair.second) << 12);
  }
...
void RiskEngine::WriteMarshallable(common::BytesOut& bytes) const {
  ...
  utils::SerializationUtils::MarshallIntHashMap<LastPriceCacheRecord>(lastPriceCache_, bytes);
