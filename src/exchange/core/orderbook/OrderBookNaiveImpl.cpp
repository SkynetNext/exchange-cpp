...
void OrderBookNaiveImpl::NewOrderPlaceGtc(common::cmd::OrderCommand* cmd) {
    ...
    // Create order record
    auto* orderRecord = new common::Order(newOrderId,
                                          price,
                                          size,
                                          filledSize,
                                          cmd->reserveBidPrice,
...
std::map<int64_t, std::unique_ptr<OrdersBucket>>* OrderBookNaiveImpl::GetBucketsByAction(
...
const std::map<int64_t, std::unique_ptr<OrdersBucket>>* OrderBookNaiveImpl::GetBucketsByAction(
    common::OrderAction action) const {
    if (action == common::OrderAction::ASK) {
        return &askBuckets_;
    } else {
        // bidBuckets_ has different comparator, need to cast
        return reinterpret_cast<const std::map<int64_t, std::unique_ptr<OrdersBucket>>*>(
            &bidBuckets_);
    }
}
