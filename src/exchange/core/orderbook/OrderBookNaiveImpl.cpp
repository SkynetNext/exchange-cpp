...
void OrderBookNaiveImpl::NewOrderPlaceGtc(common::cmd::OrderCommand* cmd) {
  ...
  // Create order record
  auto* orderRecord = new common::Order(newOrderId, price, size, filledSize, cmd->reserveBidPrice,
                                        action, cmd->uid, cmd->timestamp);
...
void OrderBookNaiveImpl::NewOrderMatchFokBudget(common::cmd::OrderCommand* cmd) {
  int64_t size = cmd->size;
...
bool OrderBookNaiveImpl::IsBudgetLimitSatisfied(common::OrderAction orderAction,
  ...
  // failure)
  return calculated == limit || (orderAction == common::OrderAction::BID ^ calculated > limit);
}
...
int64_t OrderBookNaiveImpl::TryMatchInstantly(const common::IOrder* activeOrder,
                                              MatchingRange& matchingRange,
                                              int64_t filled,
...
common::cmd::CommandResultCode OrderBookNaiveImpl::CancelOrder(common::cmd::OrderCommand* cmd) {
  ...
  if (order->action == common::OrderAction::ASK) {
    ...
    if (bucketIt == askBuckets_.end()) {
      throw std::runtime_error("Cannot find bucket for order price=" + std::to_string(price));
    }
  ...
  cmd->matcherEvent =
    eventsHelper_->SendReduceEvent(orderPrice, orderReserveBidPrice, reduceSize, true);
...
common::cmd::CommandResultCode OrderBookNaiveImpl::ReduceOrder(common::cmd::OrderCommand* cmd) {
  int64_t orderId = cmd->orderId;
  ...
  if (order->action == common::OrderAction::ASK) {
    ...
    if (bucketIt == askBuckets_.end()) {
      throw std::runtime_error("Cannot find bucket for order price=" + std::to_string(orderPrice));
    }
  ...
  if (canRemove) {
    ...
    // Use saved data to create event
    cmd->matcherEvent =
      eventsHelper_->SendReduceEvent(orderPrice, orderReserveBidPrice, reduceBy, true);
    cmd->action = orderAction;
  } else {
    order->size -= reduceBy;
...
common::cmd::CommandResultCode OrderBookNaiveImpl::MoveOrder(common::cmd::OrderCommand* cmd) {
  int64_t orderId = cmd->orderId;
  ...
  if (symbolSpec_->type == common::SymbolType::CURRENCY_EXCHANGE_PAIR
      && order->action == common::OrderAction::BID && cmd->price > order->reserveBidPrice) {
    return common::cmd::CommandResultCode::MATCHING_MOVE_FAILED_PRICE_OVER_RISK_LIMIT;
  }
...
std::shared_ptr<common::L2MarketData> OrderBookNaiveImpl::GetL2MarketDataSnapshot(int32_t size) {
...
OrderBookNaiveImpl::GetBucketsByAction(common::OrderAction action) {
  if (action == common::OrderAction::ASK) {
    ...
    // operations
    return reinterpret_cast<std::map<int64_t, std::unique_ptr<OrdersBucket>>*>(&bidBuckets_);
  }
}
...
OrderBookNaiveImpl::GetBucketsByAction(common::OrderAction action) const {
  if (action == common::OrderAction::ASK) {
    ...
    // bidBuckets_ has different comparator, need to cast
    return reinterpret_cast<const std::map<int64_t, std::unique_ptr<OrdersBucket>>*>(&bidBuckets_);
  }
}

OrderBookNaiveImpl::MatchingRange OrderBookNaiveImpl::GetMatchingRange(common::OrderAction action,
                                                                       int64_t price) {
  if (action == common::OrderAction::ASK) {
    ...
    auto shouldContinue = [price](int64_t bucketPrice) {
      return bucketPrice >= price;  // For ASK: match BID with price >= order price
    };
    ...
    auto shouldContinue = [price](int64_t bucketPrice) {
      return bucketPrice <= price;  // For BID: match ASK with price <= order price
    };
...
OrderBookNaiveImpl::OrderBookNaiveImpl(common::BytesIn* bytes,
...
void OrderBookNaiveImpl::WriteMarshallable(common::BytesOut& bytes) const {
  ...
  if (symbolSpec_ != nullptr) {
    ...
    // on non-const
    const_cast<common::CoreSymbolSpecification*>(symbolSpec_)->WriteMarshallable(bytes);
  }
  ...
  if (askBuckets_.empty()) {
    ...
    for (const auto& pair : askBuckets_) {
      ...
      oss << "  Price: " << price << " -> Bucket: " << bucket
          << " (orders: " << bucket->GetNumOrders() << ", volume: " << bucket->GetTotalVolume()
          << ")\n";
    }
  }
  return oss.str();
