 ...
 */
template <typename V>
class ArtNode48 : public IArtNode<V> {
public:
  static constexpr int NODE16_SWITCH_THRESHOLD = 12;

  explicit ArtNode48(ArtPoolContext<V>* poolContext)
    : IArtNode<V>(kArtNode48), poolContext_(poolContext) {
    indexes_.fill(-1);
    nodes_.fill(nullptr);
  }
...
template <typename V>
V* ArtNode48<V>::GetCeilingValue(int64_t key, int level) {
  if (level != nodeLevel_) {
...
template <typename V>
V* ArtNode48<V>::GetFloorValue(int64_t key, int level) {
  if (level != nodeLevel_) {
    const int64_t mask = -1LL << (nodeLevel_ + 8);
    if ((nodeKey_ & mask) > (key & mask))
      return nullptr;
...
int ArtNode48<V>::ForEach(LongObjConsumer<V>* consumer, int limit) const {
  ...
  for (int i = 0; i < 256 && numLeft > 0; i++) {
    if (indexes_[i] != -1) {
      if (nodeLevel_ == 0) {
        consumer->Accept((nodeKey_ & (-1LL << 8)) + i, static_cast<V*>(nodes_[indexes_[i]]));
        numLeft--;
      } else
        numLeft -= static_cast<IArtNode<V>*>(nodes_[indexes_[i]])->ForEach(consumer, numLeft);
    }
  }
  ...
  for (int i = 255; i >= 0 && numLeft > 0; i--) {
    if (indexes_[i] != -1) {
      if (nodeLevel_ == 0) {
        ...
        numLeft -= static_cast<IArtNode<V>*>(nodes_[indexes_[i]])->ForEachDesc(consumer, numLeft);
    }
  }
  return limit - numLeft;
}
...
template <typename V>
int ArtNode48<V>::Size(int limit) {
  if (nodeLevel_ == 0)
    return numChildren_;
  int numLeft = limit;
  for (int i = 0; i < 256 && numLeft > 0; i++)
...
template <typename V>
void ArtNode48<V>::ValidateInternalState(int level) {
  if (nodeLevel_ > level)
  ...
  for (int i = 0; i < 256; i++) {
    if (indexes_[i] != -1) {
      ...
      if (nodeLevel_ != 0)
        static_cast<IArtNode<V>*>(nodes_[indexes_[i]])->ValidateInternalState(nodeLevel_ - 8);
    }
  }
