...
class SerializationUtils {
 public:
     ...
     */
    static ankerl::unordered_dense::map<int32_t, int64_t> MergeSum(
     ...
     */
    template <typename T>
    static T* PreferNotNull(T* a, T* b) {
    ...
    static ankerl::unordered_dense::map<K, V> MergeOverride(
        const ankerl::unordered_dense::map<K, V>* a,
     ...
     * Marshall long array
     */
    static void MarshallLongArray(const std::vector<int64_t>& longs, common::BytesOut& bytes);
     ...
     */
    static ankerl::unordered_dense::map<int32_t, int64_t> ReadIntLongHashMap(
     ...
     */
    template <typename T>
    static void MarshallIntHashMap(const ankerl::unordered_dense::map<int32_t, T*>& hashMap,
                                   common::BytesOut& bytes) {
    ...
    static void MarshallIntHashMap(
        ...
        typename std::enable_if<std::is_same_v<
            decltype(std::declval<const T>().WriteMarshallable(std::declval<common::BytesOut&>())),
            void>>::type* = nullptr) {
        bytes.WriteInt(static_cast<int32_t>(hashMap.size()));
        for (const auto& pair : hashMap) {
            bytes.WriteInt(pair.first);
     ...
     */
    template <typename T>
    static ankerl::unordered_dense::map<int32_t, T*> ReadIntHashMap(
        common::BytesIn& bytes,
     ...
     */
    template <typename T>
    static void MarshallLongHashMap(const ankerl::unordered_dense::map<int64_t, T*>& hashMap,
                                    common::BytesOut& bytes) {
        bytes.WriteInt(static_cast<int32_t>(hashMap.size()));
        for (const auto& pair : hashMap) {
     ...
     */
    template <typename T>
    static ankerl::unordered_dense::map<int64_t, T*> ReadLongHashMap(
        common::BytesIn& bytes,
        std::function<T*(common::BytesIn&)> creator) {
        int length = bytes.ReadInt();
     ...
     */
    template <typename T>
    static void MarshallNullable(T* object,
                                 common::BytesOut& bytes,
     ...
     */
    template <typename T>
    static T* ReadNullable(common::BytesIn& bytesIn, std::function<T*(common::BytesIn&)> creator) {
     ...
     */
    static std::vector<uint8_t> LongsLz4ToBytes(const std::vector<int64_t>& dataArray,
     ...
     */
    static std::vector<int64_t> BytesToLongArrayLz4(const std::vector<uint8_t>& bytes, int padding);
     ...
     */
    static std::vector<int64_t> BytesToLongArray(const std::vector<uint8_t>& bytes, int padding);
     ...
     */
    template <typename K, typename V>
    static void MarshallGenericMap(const std::map<K, V>& map,
                                   common::BytesOut& bytes,
     ...
     */
    template <typename T>
    static void MarshallNullable(const T* object,
                                 common::BytesOut& bytes,
     ...
     */
    template <typename T>
    static void MarshallList(const std::vector<T*>& list,
                             common::BytesOut& bytes,
                             std::function<void(T*, common::BytesOut&)> elementMarshaller) {
