...
public:
  ...
  int32_t quoteCurrency =
      0; // quote/counter currency (OR futures contract currency)
  int64_t baseScaleK =
      0; // base currency amount multiplier (lot size in base currency units)
  int64_t quoteScaleK =
      0; // quote currency amount multiplier (step size in quote currency units)

  // fees per lot in quote currency units
  int64_t takerFee =
      0; // TODO check invariant: taker fee is not less than maker fee
  int64_t makerFee = 0;
  ...
  CoreSymbolSpecification(int32_t symbolId, SymbolType type,
                          int32_t baseCurrency, int32_t quoteCurrency,
                          int64_t baseScaleK, int64_t quoteScaleK,
