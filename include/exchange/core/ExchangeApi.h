...
class IExchangeApi {
 public:
     ...
     */
    virtual std::future<common::cmd::CommandResultCode> SubmitCommandAsync(
        common::api::ApiCommand* cmd) = 0;

    /**
     * Submit command async with full response (returns OrderCommand future)
     * Matches Java submitCommandAsyncFullResponse
     */
    virtual std::future<common::cmd::OrderCommand> SubmitCommandAsyncFullResponse(
        common::api::ApiCommand* cmd) = 0;

    /**
     * Submit commands synchronously
     */
    virtual void SubmitCommandsSync(const std::vector<common::api::ApiCommand*>& cmds) = 0;

    /**
     * Submit commands in batch (true batch write to ring buffer)
     * Uses next(n) and publish(lo, hi) for better performance
     */
    virtual void SubmitCommandsBatch(const std::vector<common::api::ApiCommand*>& cmds) = 0;
    ...
    ProcessReportAny(int32_t queryTypeId, std::vector<uint8_t> queryBytes, int32_t transferId) = 0;
     ...
     */
    virtual std::future<std::shared_ptr<common::L2MarketData>> RequestOrderBookAsync(
        int32_t symbolId,
        int32_t depth) = 0;
     ...
     * @param word4 Word 4
     */
    virtual void BinaryData(int32_t serviceFlags,
                            int64_t eventsGroup,
                            int64_t timestampNs,
                            int8_t lastFlag,
                            int64_t word0,
    ...
    virtual void PlaceOrderReplay(int32_t serviceFlags,
                                  int64_t eventsGroup,
                                  int64_t timestampNs,
                                  int64_t orderId,
                                  int32_t userCookie,
    ...
    virtual void MoveOrderReplay(int32_t serviceFlags,
                                 int64_t eventsGroup,
                                 int64_t timestampNs,
                                 int64_t price,
                                 int64_t orderId,
    ...
    virtual void CancelOrderReplay(int32_t serviceFlags,
                                   int64_t eventsGroup,
                                   int64_t timestampNs,
                                   int64_t orderId,
                                   int32_t symbol,
    ...
    virtual void ReduceOrderReplay(int32_t serviceFlags,
                                   int64_t eventsGroup,
                                   int64_t timestampNs,
                                   int64_t reduceSize,
                                   int64_t orderId,
    ...
    virtual void BalanceAdjustmentReplay(int32_t serviceFlags,
                                         int64_t eventsGroup,
                                         int64_t timestampNs,
                                         int64_t uid,
    ...
    virtual void CreateUserReplay(int32_t serviceFlags,
                                  int64_t eventsGroup,
                                  int64_t timestampNs,
                                  int64_t userId) = 0;

    // Match Java: suspendUser(serviceFlags, eventsGroup, timestampNs, ...)
    virtual void SuspendUserReplay(int32_t serviceFlags,
                                   int64_t eventsGroup,
                                   int64_t timestampNs,
                                   int64_t userId) = 0;

    // Match Java: resumeUser(serviceFlags, eventsGroup, timestampNs, ...)
    virtual void ResumeUserReplay(int32_t serviceFlags,
                                  int64_t eventsGroup,
                                  int64_t timestampNs,
                                  int64_t userId) = 0;
...
class ExchangeApi : public IExchangeApi {
 public:
     ...
     * Submit command async (returns future)
     */
    std::future<common::cmd::CommandResultCode> SubmitCommandAsync(
        common::api::ApiCommand* cmd) override;

    /**
     * Submit command async with full response (returns OrderCommand future)
     * Matches Java submitCommandAsyncFullResponse
     */
    std::future<common::cmd::OrderCommand> SubmitCommandAsyncFullResponse(
        common::api::ApiCommand* cmd) override;

    /**
     * Submit commands synchronously
     */
    void SubmitCommandsSync(const std::vector<common::api::ApiCommand*>& cmds) override;

    /**
     * Submit commands in batch (true batch write to ring buffer)
     * Uses next(n) and publish(lo, hi) for better performance
     */
    void SubmitCommandsBatch(const std::vector<common::api::ApiCommand*>& cmds) override;
    ...
    std::future<std::unique_ptr<R>> ProcessReport(std::unique_ptr<Q> query, int32_t transferId);
     ...
     */
    std::future<std::vector<std::vector<uint8_t>>> ProcessReportAny(int32_t queryTypeId,
     ...
     * OrderCommand::marketData)
     */
    std::future<std::shared_ptr<common::L2MarketData>> RequestOrderBookAsync(
        int32_t symbolId,
     ...
     * @param word4 Word 4
     */
    void BinaryData(int32_t serviceFlags,
                    int64_t eventsGroup,
                    int64_t timestampNs,
                    int8_t lastFlag,
                    int64_t word0,
    ...
    // Replay methods for journaling (with serviceFlags and eventsGroup)
    void PlaceOrderReplay(int32_t serviceFlags,
                          int64_t eventsGroup,
                          int64_t timestampNs,
                          int64_t orderId,
                          int32_t userCookie,
    ...
    void MoveOrderReplay(int32_t serviceFlags,
                         int64_t eventsGroup,
                         int64_t timestampNs,
                         int64_t price,
                         int64_t orderId,
    ...
    void CancelOrderReplay(int32_t serviceFlags,
                           int64_t eventsGroup,
                           int64_t timestampNs,
                           int64_t orderId,
                           int32_t symbol,
    ...
    void ReduceOrderReplay(int32_t serviceFlags,
                           int64_t eventsGroup,
                           int64_t timestampNs,
                           int64_t reduceSize,
                           int64_t orderId,
    ...
    void BalanceAdjustmentReplay(int32_t serviceFlags,
                                 int64_t eventsGroup,
                                 int64_t timestampNs,
                                 int64_t uid,
                                 int64_t transactionId,
    ...
    void CreateUserReplay(int32_t serviceFlags,
                          int64_t eventsGroup,
                          int64_t timestampNs,
                          int64_t userId) override;

    void SuspendUserReplay(int32_t serviceFlags,
                           int64_t eventsGroup,
                           int64_t timestampNs,
                           int64_t userId) override;

    void ResumeUserReplay(int32_t serviceFlags,
                          int64_t eventsGroup,
                          int64_t timestampNs,
                          int64_t userId) override;
 ...
 private:
    ...
    using PromiseMap =
        tbb::concurrent_hash_map<int64_t, std::promise<common::cmd::CommandResultCode>>;
    ...
    using ReportPromiseMap = tbb::concurrent_hash_map<int64_t, ReportResultPromise>;
    ...
    using OrderBookPromiseMap =
        tbb::concurrent_hash_map<int64_t, std::promise<std::shared_ptr<common::L2MarketData>>>;
    ...
    using FullResponsePromiseMap =
        tbb::concurrent_hash_map<int64_t, std::promise<common::cmd::OrderCommand>>;
...
std::future<std::unique_ptr<R>> ProcessReportHelper(IExchangeApi* api,
    ...
    auto sectionsFuture =
        api->ProcessReportAny(query->GetReportTypeCode(), std::move(queryBytes), transferId);
    ...
    for (const auto& section : sections) {
        if (!section.empty()) {
            sectionBytesOwners.push_back(std::make_unique<common::VectorBytesIn>(section));
