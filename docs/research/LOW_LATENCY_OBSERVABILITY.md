...
| **亚微秒开销** | 单消息追踪开销 <1µs |
| **无偏延迟计算** | 通过消差算法抵消时钟偏差 |
| **分级采样** | 漏桶 rate=10 burst=10（10 r/s），低负载全采、高负载每 100ms 一条，统一配置 |
...
### 3.7 采样策略（漏桶 rate=10r/s burst=10）

**采样决策仅在 Gateway**：用 **漏桶（Leaky Bucket）** 控制采样率；若本次允许采样则填充 TraceHeader，否则不带；下游仅看有无头。本场景不需要 Nginx 的 burst 排队语义（突发请求进入队列延迟处理），只需决定采样/不采样，天然等价于 `nodelay`。

**漏桶算法**（Nginx 风格）：用 `excess` 表示「欠债」（已消耗但未还的额度），单位毫秒。参考实现：

```text
// 状态：last_ms, excess（欠债，单位 ms）
// 参数：rate（r/s）、burst（突发容量）
// 推导：delay = 1000 / rate（每请求消耗的额度），limit = burst * delay（最大欠债）

now_ms = monotonic_now_ms();
ms = now_ms - last_ms;
if (ms < 0) ms = 0;
last_ms = now_ms;                    // 始终更新时间（关键！）
excess = excess - ms;                // 时间流逝，还债
if (excess < 0) excess = 0;
if (excess + delay > limit)          // 欠债将超限
    return 不采样;                   // 拒绝，不增加欠债
excess = excess + delay;             // 采样，增加欠债
return 采样;
```

**典型配置**：

| rate | burst | delay | limit | 行为 |
|------|-------|-------|-------|------|
| 10 | 0 | 100ms | 0 | **完全不采集**（limit=0，每次拒绝） |
| 10 | 1 | 100ms | 100ms | 匀速、无突发：最多每 100ms 采 1 条 |
| **10** | **10** | **100ms** | **1000ms** | **推荐**：低负载（<10 r/s）请求基本都能被记录，10 条集中在前半秒也可全采到；高负载稳态每 100ms 采样一条，均匀分布在时间上 |
| 10 | 20 | 100ms | 2000ms | 更宽松：允许瞬间 20 条突发 |

**环境**：开发/测试/预发/灰度/生产统一使用 **rate=10 burst=10**，应对各种负载。
...
| 采样决策 | ~5-10ns | Gateway 侧，毫秒时间差 + 欠债比较（3.7 漏桶），O(1) 整数运算 |
