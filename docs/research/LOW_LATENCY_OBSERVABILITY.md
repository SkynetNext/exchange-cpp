...
**配置**：**rate=10（10 r/s）、burst=10** → `delay=100ms`、`limit=1000ms`。

- **低负载**（<10 r/s）：请求基本都能被记录，10 条集中在前半秒也可全采到
- **高负载**：稳态每 100ms 采样一条，均匀分布在时间上

| 环境 | 采样策略 | 说明 |
|------|----------|------|
| **开发/测试/预发/灰度/生产** | rate=10 burst=10 | 统一配置，应对各种环境 |
