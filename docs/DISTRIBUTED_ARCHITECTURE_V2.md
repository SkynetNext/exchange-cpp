...
| 指标 | 目标值 | 测量方法 |
|------|--------|----------|
| **撮合延迟 (P50)** | ≤ 5μs | 纯撮合引擎处理时间 |
| **撮合延迟 (P99)** | ≤ 10μs | 同上 |
| **端到端延迟 (P50)** | ≤ 50μs | Gateway → 撮合完成 (不含 Raft) |
| **端到端延迟 (P99)** | ≤ 100μs | 同上 |
| **端到端延迟 (P50, 含 Raft)** | ≤ 100μs | Gateway → 撮合完成 (含 Raft 同步) |
| **端到端延迟 (P99, 含 Raft)** | ≤ 300μs | 同上 |
| **吞吐量** | ≥ 4M orders/sec | 单撮合集群  |
| **故障切换时间** | ≤ 500ms | Leader 宕机到新 Leader 可用 |
| **数据持久性** | 99.999999% | 已提交订单不丢失 |
| **可用性** | 99.99% | 年度停机 < 52.6 分钟 |

> **延迟目标说明**:
> - **撮合延迟**: 纯内存操作，与 LMAX Disruptor 架构一致
> - **端到端 (不含 Raft)**: 含网络传输 (Aeron UDP ~25μs 单程)，适用于异步复制模式
> - **端到端 (含 Raft)**: 含 Raft 同步复制 (~50μs)，适用于强一致性模式
> - 详细延迟分解见 [6.1 延迟分解](#61-延迟分解)
...
│                              订单处理延迟路径                                                 │
│                                                                                             │
│   Client                Trading Service              Matching Engine              Client    │
│     │                        │                            │                         │       │
│     │   ┌────────────────────┼────────────────────────────┼─────────────────────────┤       │
│     │   │                    │                            │                         │       │
│     │   │  ① Token 验证      │                            │                         │       │
...
| 阶段 | P50 | P99 | P99.9 | 说明 |
|------|-----|-----|-------|------|
| ① Token 验证 (LRU 命中) | 100ns | 500ns | 1μs | 本地缓存 |
| ① Token 验证 (Redis) | 100μs | 500μs | 1ms | 缓存未命中时 |
| ② 冻结检查 | 200ns | 500ns | 1μs | 本地内存操作 |
| ③ Raft 日志 (交易服务) | 50μs | 200μs | 500μs | **异步复制，不阻塞关键路径** |
| ④ 网络 (→撮合) | 25μs | 50μs | 100μs | Aeron UDP 单程 |
| ⑤ 撮合执行 | 2μs | 5μs | 10μs | 纯内存操作 |
| ⑥ Raft 日志 (撮合) | 50μs | 200μs | 500μs | **同步复制，等待多数确认** |
| ⑦ 网络 (←撮合) | 25μs | 50μs | 100μs | Aeron UDP 单程 |
| ⑧ 清算处理 | 200ns | 500ns | 1μs | 本地内存，异步 |
...
┌─────────────────────────────────────────────────────────────────────────────────────────────┐
│                                                                                             │
│   模式 A: 不含 Raft 同步 (异步复制，最低延迟)                                                 │
│   ════════════════════════════════════════════                                              │
│                                                                                             │
│   ① Token    ② 冻结     ④ 网络→    ⑤ 撮合     ⑦ 网络←    ⑧ 清算                            │
│   ┌───┐     ┌───┐      ┌────┐     ┌───┐      ┌────┐     ┌───┐                              │
│   │0.1│  +  │0.2│  +   │ 25 │  +  │ 2 │  +   │ 25 │  +  │0.2│  =  ~52μs (P50)              │
│   └───┘     └───┘      └────┘     └───┘      └────┘     └───┘                              │
│    μs        μs          μs        μs          μs        μs                                 │
│                                                                                             │
│   ┌─────────────────────────────────────────────────────────────────────────────────────┐  │
│   │  P50: ~52μs  │  P99: ~106μs  │  P99.9: ~212μs                                       │  │
│   └─────────────────────────────────────────────────────────────────────────────────────┘  │
│                                                                                             │
├─────────────────────────────────────────────────────────────────────────────────────────────┤
│                                                                                             │
│   模式 B: 含 Raft 同步 (强一致性，数据持久化)                                                 │
│   ════════════════════════════════════════════                                              │
│                                                                                             │
│   ① Token    ② 冻结     ④ 网络→    ⑤ 撮合     ⑥ Raft     ⑦ 网络←    ⑧ 清算                 │
│   ┌───┐     ┌───┐      ┌────┐     ┌───┐      ┌────┐     ┌────┐     ┌───┐                   │
│   │0.1│  +  │0.2│  +   │ 25 │  +  │ 2 │  +   │ 50 │  +  │ 25 │  +  │0.2│  =  ~102μs (P50) │
│   └───┘     └───┘      └────┘     └───┘      └────┘     └────┘     └───┘                   │
│    μs        μs          μs        μs          μs         μs        μs                      │
│                                                                                             │
│   ┌─────────────────────────────────────────────────────────────────────────────────────┐  │
│   │  P50: ~102μs  │  P99: ~306μs  │  P99.9: ~712μs                                      │  │
│   └─────────────────────────────────────────────────────────────────────────────────────┘  │
│                                                                                             │
└─────────────────────────────────────────────────────────────────────────────────────────────┘
```
...
| 优化项 | 技术手段 | 效果 |
|--------|----------|------|
| **Token 验证** | 本地 LRU 缓存 (TTL 10s) | 100μs → 100ns (1000x) |
| **冻结/清算** | 按用户分片，本地操作 | 无网络开销 |
| **网络传输** | Aeron UDP + 零拷贝 | ~25μs 单程 (同机房) |
| **撮合引擎** | Disruptor + 对象池 + ART Tree | ~2μs P50 |
| **Raft 复制** | 异步模式可选 | 关键路径减少 50μs |
