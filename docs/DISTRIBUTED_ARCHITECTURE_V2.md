...
| 指标 | 目标值 | 测量方法 |
|------|--------|----------|
| **下单延迟 (P50)** | ≤ 10μs | 从 Gateway 收到请求到撮合完成 |
| **下单延迟 (P99)** | ≤ 50μs | 同上 |
| **下单延迟 (P99.9)** | ≤ 100μs | 同上 |
| **吞吐量** | ≥ 1M orders/sec | 单撮合集群 |
...
总延迟分解 (P50):
├─ Token 验证:     ~100ns  (本地 LRU 命中)
├─ 冻结检查:       ~200ns  (本地内存操作)
├─ 网络 (→撮合):   ~25μs   (Aeron UDP 单程)
├─ 撮合执行:       ~2μs    (内存操作)
├─ 网络 (←撮合):   ~25μs   (Aeron UDP 单程)
├─ 清算处理:       ~200ns  (本地内存操作，异步)
└─ 总计:           ~52μs   (端到端)
```
...
| 阶段 | P50 | P99 | P99.9 | 说明 |
|------|-----|-----|-------|------|
| **Token 验证** | 100ns | 500ns | 1μs | 本地 LRU 命中 |
| **Token 验证** | 100μs | 500μs | 1ms | Redis 查询 (未命中) |
| **冻结检查** | 200ns | 500ns | 1μs | 本地内存操作 |
| **Raft 日志 (交易服务)** | 50μs | 200μs | 500μs | 异步复制，不阻塞 |
| **网络 (→撮合)** | 25μs | 50μs | 100μs | Aeron UDP 单程 |
| **撮合执行** | 2μs | 5μs | 10μs | 内存操作 |
| **Raft 日志 (撮合)** | 50μs | 200μs | 500μs | 同步复制，等待多数 |
| **网络 (←撮合)** | 25μs | 50μs | 100μs | Aeron UDP 单程 |
| **清算处理** | 200ns | 500ns | 1μs | 本地内存操作 |
| **端到端 (不含 Raft)** | ~52μs | ~106μs | ~212μs | 冻结 + 网络 + 撮合 |
| **端到端 (含 Raft)** | ~102μs | ~306μs | ~712μs | 含 Raft 同步复制 |
