...
| 网络 (→撮合) | ~9μs | **~1.5μs** | **~2.5μs** | 单程传输；阿里云官方 ≤5μs RTT |
...
| Raft 日志同步 | ~36μs | **~3μs** | **~5μs** | RTT；阿里云官方上限 |
| **端到端 (含Raft)** | **~56.5μs** | **~8.5μs** | **~12.5μs** | DPDK→eRDMA 提升 4.5x |

> **数据来源**：阿里云 eRDMA ≤5μs 为官方宣传上限（神龙4.0），可作为 P99 参考。网络 RTT 按 5μs 均分；Raft 同步复用同网络。
