...
| **端到端延迟 (含 Raft)** | ≤ 100μs | Gateway → 撮合完成，同步复制 (2 段 Raft) |
...
> - **端到端 (含 Raft)**: 上述 + Raft 同步 2×36μs ≈ 96μs（冻结 + 撮合各 1 RTT）
...
| 阶段 | Aeron Premium (DPDK) | RDMA Two-sided (实测) | 阿里云 eRDMA (RoCE v2) | 说明 |
|------|---------------------|----------------------|------------------------|------|
| Token 验证 | ~500ns | ~500ns | ~500ns | 本地 LRU 命中 |
| 冻结检查 | ~500ns | ~500ns | ~500ns | 本地内存操作 |
| Raft 同步 (冻结) | ~36μs | **~3μs** | **~5μs** | 冻结持久化，1 RTT |
| 网络 (→撮合) | ~9μs | **~1.5μs** | **~2.5μs** | 单程传输 |
| 撮合执行 | ~1μs | ~1μs | ~1μs | 内存操作 (优化后 P99) |
| Raft 同步 (撮合) | ~36μs | **~3μs** | **~5μs** | 撮合持久化，1 RTT |
| 网络 (←撮合) | ~9μs | **~1.5μs** | **~2.5μs** | 单程传输 |
| 清算处理 | ~500ns | ~500ns | ~500ns | 本地内存操作，异步 |
| **端到端 (不含Raft)** | **~20.5μs** | **~5.5μs** | **~7.5μs** | DPDK→eRDMA 提升 2.7x |
| **端到端 (含Raft)** | **~92.5μs** | **~11.5μs** | **~17.5μs** | 含 2 段 Raft RTT |

> **数据来源**：阿里云 eRDMA ≤5μs 为官方宣传上限（神龙4.0），可作为 P99 参考。网络 RTT 按 5μs 均分；Raft 同步需 2 次（冻结 + 撮合），各 1 RTT。
...
| **阿里云 eRDMA (RoCE v2)** | 神龙架构 | ~2.5μs | **≤5μs** | VPC 复用 | 国内云端首选 |
...
| 冻结检查 | ~500ns | 本地内存操作 |
| Raft 同步 (冻结) | ~36μs | 冻结持久化，同步复制 |
| 网络 (→撮合) | ~9μs | Aeron Premium 单程 |
| 撮合执行 | ~5μs | 纯内存操作 |
| Raft 同步 (撮合) | ~36μs | 撮合持久化，同步复制 |
| 网络 (←撮合) | ~9μs | Aeron Premium 单程 |
| 清算处理 | ~500ns | 本地内存，异步 |
...
| **含 Raft** | 0.5 + 0.5 + 36 + 9 + 5 + 36 + 9 + 0.5 μs | **~96μs** |
