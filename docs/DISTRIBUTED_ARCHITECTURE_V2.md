...
| 阶段 | Aeron Premium (DPDK) | RDMA (ConnectX-7) | 说明 |
|------|---------------------|-------------------|------|
| Token 验证 | ~500ns | ~500ns | 本地 LRU 命中 |
| 冻结检查 | ~500ns | ~500ns | 本地内存操作 |
| 网络 (→撮合) | ~9μs | **< 0.6μs** | 单程传输 |
| 撮合执行 | ~1.8μs | ~1.8μs | 内存操作 (实测 5M TPS P99) |
| 网络 (←撮合) | ~9μs | **< 0.6μs** | 单程传输 |
| 清算处理 | ~500ns | ~500ns | 本地内存操作，异步 |
| **端到端 (不含Raft)** | **~21μs** | **~4.5μs** | **提升 4.7x** |
| Raft 日志同步 | ~36μs | **< 1.2μs** | RTT，强一致性保证 |
| **端到端 (含Raft)** | **~57μs** | **~5.7μs** | **提升 10x** |

**网络方案对比**

| 方案 | 硬件 | 单程延迟 | RTT | 带宽 | 成本 | 适用场景 |
|------|------|---------|-----|------|------|---------|
| **Aeron Premium (DPDK)** | 通用网卡 | ~9μs | ~18μs | 10-100 Gbps | 中 | 云端/IDC 通用方案 |
| **RDMA (InfiniBand/RoCEv2)** | NVIDIA ConnectX-7 | **< 0.6μs** | **< 1.2μs** | 400 Gbps | 高 | HFT/超低延迟场景 |
