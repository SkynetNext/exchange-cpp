...
```
纳秒级（ns）：
├── L1 cache hit: 0.5ns (1-2周期) ⭐ 最快
├── L2 cache hit: 3-5ns (9-15周期) [最小值: 3ns]
├── 直接函数调用: ~6ns (~18周期)
├── L3 cache hit: 10-20ns (30-60周期) [最小值: 10ns]
├── 虚函数调用: ~14ns (~42周期)
├── StoreLoad barrier: 15-25ns (45-75周期) [最小值: 15ns]
├── Lock-free CAS: 15-30ns (45-90周期) [最小值: 15ns]
├── std::function调用: ~20-33ns (~60-100周期) [最小值: 20ns]
├── Mutex lock/unlock: 25-50ns (75-150周期) [最小值: 25ns]
├── Disruptor: 50-200ns (150-600周期) [最小值: 50ns]
├── LMAX Mean Latency: 52ns (基准)
└── DDR4主内存: 60-80ns (180-240周期) [最小值: 60ns]

微秒级（μs）：
├── Aeron IPC: 300ns-1μs (900-3000周期) [最小值: 0.3μs]
├── localhost UDP: 500ns-2μs (1500-6000周期) [最小值: 0.5μs]
├── RDMA (InfiniBand): 1-10μs (3K-30K周期) [最小值: 1μs] ⭐ 最低网络延迟
├── Thread context switch: 1-5μs (3K-15K周期) [最小值: 1μs]
├── RDMA (RoCE): 2-15μs (6K-45K周期) [最小值: 2μs]
├── Cilium eBPF转发: 5-50μs (15K-150K周期) [最小值: 5μs]
├── localhost TCP: 5-10μs (15K-30K周期) [最小值: 5μs]
├── 云商同机房网络: 10-100μs (30K-300K周期) [最小值: 10μs]
├── NVMe SSD随机读取: 20-100μs (60K-300K周期) [最小值: 20μs]
└── SATA SSD随机读取: 50-100μs (150K-300K周期) [最小值: 50μs]

毫秒级（ms）：
├── 跨AZ网络: 0.1-1ms (300K-3M周期) [最小值: 0.1ms]
├── Cilium eBPF跨AZ RTT: ~0.55ms (~1.65M周期)
├── HDD (15000 RPM): ~2ms (~6M周期)
└── HDD (7200 RPM): 8-12ms (24M-36M周期) [最小值: 8ms] ⚠️ 最慢
```
---
