...
### C++ 内存顺序核心记忆表（按线程/变量/约束维度）

| 内存顺序类型 | 单线程约束（重排规则） | 跨线程约束（可见性+顺序性） | 绑定对象 | 核心特性（精准记忆点） | 低延迟场景适用性 |
|-------------|----------------------|---------------------------|---------|---------------------|-----------------|
| `acquire`（原子读专用） | **单向约束**：原子读之后的所有内存操作（普通/原子），不能重排到原子读之前；原子读之前的操作，可自由重排到原子读之后。 | **局部可见性**：必须与生产者线程的 `release` 配对；消费者线程 `acquire` 读，能看到生产者 `release` 写之前的所有内存操作。 | 绑定原子变量的读操作（`load(acquire)`） | 「后不往前」+「生产者-消费者局部同步」 | ✅ 首选（无锁队列/撮合引擎核心同步语义） |
| `release`（原子写专用） | **单向约束**：原子写之前的所有内存操作（普通/原子），不能重排到原子写之后；原子写之后的操作，可自由重排到原子写之前。 | **局部可见性**：必须与消费者线程的 `acquire` 配对；生产者线程 `release` 写，会「发布」之前的所有操作给配对的 `acquire` 读。 | 绑定原子变量的写操作（`store(release)`） | 「前不往后」+「发布-获取局部同步」 | ✅ 首选（与 `acquire` 配对，x86 无额外指令开销） |
| `acq_rel`（原子读写专用） | **双向约束**：原子操作前后的所有内存操作，都不能跨越原子操作重排；等价于「`acquire` + `release`」的单线程组合。 | **局部双向同步**：既可以作为 `release` 与下游 `acquire` 配对，也可以作为 `acquire` 与上游 `release` 配对；无全局顺序保证。 | 绑定原子变量的读写操作（`fetch_add(acq_rel)`） | 「双向防重排」+「局部双向同步」 | ⚠️ 仅用于原子读写（如计数器），避免滥用 |
| `seq_cst`（最强语义） | **双向约束**：与 `acq_rel` 完全一致（原子操作前后操作不能跨屏障重排）。 | 1. 包含 `acq_rel` 的局部同步能力；2. **全局全序特性**：所有线程对所有 `seq_cst` 原子操作的执行顺序，达成唯一共识；非 `seq_cst` 操作不参与全局顺序。 | 绑定原子变量的任意操作（`load/store/...(seq_cst)`） | 「`acq_rel` 双向约束」+「全局操作顺序共识」 | ❌ 绝对避免（x86 需 `mfence` 指令，增加 10-20ns 延迟） |
| `relaxed`（最弱语义） | **无任何重排约束**：仅保证原子操作自身的原子性，不影响前后操作顺序。 | **无可见性保证**：仅保证原子变量的读写是原子的，不保证跨线程操作的顺序/可见性。 | 绑定原子变量的任意操作 | 「仅原子性，无任何顺序/可见性保证」 | ✅ 用于无需同步的原子变量（如独立计数器） |

### 补充关键记忆点（精准到线程/变量）

#### 1. 跨线程同步的前提：必须配对

- 只有 `release`（生产者写）+ `acquire`（消费者读）才能实现跨线程可见性；单方面的 `acquire`/`release` 无意义。
- `seq_cst` 的全局全序，仅对 `seq_cst` 原子操作生效，普通变量/非 `seq_cst` 原子操作不参与。

#### 2. 显式屏障（`atomic_thread_fence`）的简化记忆

| 显式屏障类型 | 等价语义 | 核心限制 |
|-------------|---------|---------|
| `fence(acquire)` | 与原子读 `acquire` 等价 | 需配合原子操作才能同步 |
| `fence(release)` | 与原子写 `release` 等价 | 需配合原子操作才能同步 |
| `fence(seq_cst)` | 无 `seq_cst` 原子操作锚点 → 等价 `acq_rel`；有 `seq_cst` 原子操作锚点 → 才具备全局全序 | 需锚点才能发挥全序作用 |

#### 3. 变量维度的核心规则

- 内存顺序约束的是「原子操作与普通内存操作的顺序关系」，普通变量本身无原子性，必须被 `release-acquire` 包裹才能安全并发访问。
- 全局全序仅针对 `seq_cst` 原子操作，与普通变量无关。
