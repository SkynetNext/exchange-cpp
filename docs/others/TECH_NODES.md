...
核心理念：**从"人对人"的模糊评审，转变为"规则+工具+清单"的标准化评审**。
...
|------|--------|--------|------|
| **S 级** | < 20 行 | 1 人快审 | ≤ 1h |
| **M 级** | 20-200 行 | 2 人模块审 | ≤ 1 工作日 |
| **L 级** | > 200 行 | 3+ 人会审 | ≤ 2 工作日 |

**强制触发**：核心代码需架构师参与，热路径需附 Benchmark，CI 未通过直接驳回。

#### 3.2.2 评审检查清单

详见 PR 模板：[`.github/pull_request_template.md`](../../.github/pull_request_template.md)

提交人创建 PR 时自动加载，涵盖：
- **变更类型** & **影响范围**（决定评审级别）
- **测试覆盖** & **性能 Benchmark**
- **Checklist**：代码质量、内存安全、并发正确性、热路径性能

#### 3.2.3 问题分类

| 类型 | 处理 |
|------|------|
| 🔴 **Blocker** | 必须修改（内存泄漏、逻辑错误、性能退化） |
| 🟡 **Major** | 强烈建议（可读性、注释） |
| 🟢 **Minor** | 可选（命名、风格偏好） |

**合并条件**：Blocker 清零 + 全员 Approved + CI 通过 + Benchmark 无退化
...
```
代码提交 → CI 检查 → [通过] → 分级评审 → 修改 → 合并
              ↓
           [失败] → 驳回
```

**平台要求**：所有评审在 GitHub PR 完成，禁止线下口头同意，评审记录可追溯。
### 3.3 架构文档化
