...
### 测试范围
- 吞吐量范围：0.2M - 8.7M TPS
- 测试配置：Group=256, msgsInGroupLimit=256

### 核心批次大小统计数据

**ME/R1/R2 处理器批次大小统计表**：

| rate | P50 | P90 | P95 | P99 | P99.9 | Max |
|------|-----|-----|-----|-----|-------|-----|
| 200K | 1 | 1 | 1 | 1 | 1 | 256 |
| 1M | 1 | 1 | 1 | 1 | 1 | 256 |
| 2M | 1 | 1 | 1 | 1 | 3 | 256 |
...
| 4M | 2 | 3 | 3 | 5 | 6 | 256 |
| 5M | 3 | 4 | 5 | 7 | 10 | 256 |
| 6M | 7 | 10 | 12 | 15 | 30 | 256 |
| 7M | 17 | 26 | 30 | 39 | 67 | 256 |
| 8M | 37 | 66 | 81 | 140 | 256 | 256 |
| 8.7M | 256 | 256 | 256 | 256 | 256 | 256 |

**GroupingProcessor 批次大小统计表**：
...
| 7M | 1 | 1 | 1 | 5 | 6 | 25 |
| 8M | 1 | 1 | 1 | 6 | 8 | 121 |
| 8.7M | 1 | 1 | 1 | 10 | 13 | 155 |

### GroupingProcessor 分组逻辑

**分组机制**：由 `GroupingProcessor` 负责将消息分组，通过 `cmd->eventsGroup` 字段标记组号。

**分组条件（OR 关系，满足任一条件即切换组）**：
1. **达到消息数量上限**：`msgsInGroup >= msgsInGroupLimit_`（本次测试：256条）
2. **超过时间限制**：`t > groupLastNs`（本次测试：10µs）

**分组逻辑**：
- 每条消息被分配一个 `eventsGroup` 值（`groupCounter`）
- 当达到256条消息或超过10µs时，`groupCounter++`，后续消息进入新组
- 低吞吐量下：消息间隔 > 10µs，通常由时间限制触发分组，组内消息数 < 256
- 高吞吐量下：消息快速到达，通常由数量限制触发分组，组内消息数 = 256

**分组与分 Shard 的关系**：
- **分组（eventsGroup）**：用于流水线并行，让 R1 和 R2 可以并行处理不同的组
- **分 Shard**：R1 通过 `(uid & shardMask) == shardId` 过滤，ME 通过 `(symbolId & shardMask) == shardId` 过滤
- **独立关系**：分组逻辑不涉及分 shard，但分 shard 会影响每个实例看到的批次大小
- **本次测试**：只有 1 个 ME 和 1 个 R1（shardId=0），所有消息都经过它们，批次大小 = eventsGroup 的大小

### 关键发现

**批次大小变化趋势**：
- **≤3M TPS**：ME/R1/R2 批次大小 P50/P99=1，Max=256（启动时一次性处理256条）
- **3-5M TPS**：批次大小 P50=1-3, P99=2-9
- **5-7M TPS**：批次大小 P50=3-17, P99=9-39
- **8M TPS**：批次大小 P50=37, P99=140
- **8.7M TPS**：批次大小 P50=256（首次达到），系统达到瓶颈

**Max=256 的原因**：
- 启动阶段：线程启动时一次性处理256条消息（都在同一组内）
- 高负载阶段：8M+ 时系统达到瓶颈，每次循环都处理满256条消息
...
## Sequence 缓存行对齐优化（2026-01-20）

### 优化内容

**原实现**：手动 padding，sizeof = 132B，实际分配 256B
**新实现**：`class alignas(128) Sequence { std::atomic<int64_t> value_; }`，sizeof = 128B

### 性能对比（TestLatencyMargin，三轮测试）

| TPS | 指标 | 优化前 | 测试1 (17:36) | 测试2 (20:22) | 测试3 (20:44) | 平均改善 |
|-----|------|--------|---------------|---------------|---------------|----------|
| 1M | P50 | 0.50µs | 0.50µs | 0.50µs | 0.50µs | - |
| 1M | P99 | 0.73µs | 0.71µs | 0.72µs | 0.71µs | -3% |
| 2M | P50 | 0.49µs | 0.49µs | 0.49µs | 0.49µs | - |
| 2M | P99 | 0.73µs | 0.72µs | 0.72µs | 0.72µs | -1% |
| 4M | P50 | 0.82µs | 0.83µs | 0.82µs | 0.82µs | - |
| 4M | P99 | 1.6µs | 1.58µs | 1.56µs | 1.56µs | -3% |
| 6M | P50 | 1.25µs | 1.27µs | 1.25µs | 1.25µs | - |
| 6M | P99 | 4.6µs | 3.8µs | 3.7µs | 3.7µs | **-18%** |
| 8M | P50 | 3.8µs | 3.9µs | 3.8µs | 3.8µs | - |
| 8M | P99 | 15.2µs | 13.3µs | 12.9µs | 13.1µs | **-14%** |
| 9M | P50 | 8.4µs | 8.5µs | 8.4µs | 8.3µs | - |
| 9M | P99 | 68µs | 28.5µs | 24.7µs | 23.9µs | **-62%** |
| 9M | P99.9 | 197µs | 170µs | 166µs | 162µs | **-16%** |

### 高百分位改善（9M TPS）

| 指标 | 优化前 | 优化后(avg) | 改善 |
|------|--------|-------------|------|
| P95 | 16.1µs | 14.5µs | -10% |
| P99 | 68µs | 25.7µs | **-62%** |
| P99.9 | 197µs | 166µs | **-16%** |
| P99.99 | 209µs | 178µs | **-15%** |

### 结论

- 低负载（≤4M）：无明显变化
- 中高负载（6-8M）：P99 改善 14-18%
- 极限负载（9M）：**P99 改善 62%**（68µs → 25.7µs）
- 原理：`alignas(128)` 对齐 Intel L2 Spatial Prefetcher 的 128B 预取块，避免伪共享
