...
## TwoStepMasterProcessor 优化（2025-12-30）

**优化内容**：
1. **Group 大小调整**：从 Java 版本的 256 改为 32
2. **最后一个组触发逻辑**：修复 Java 版本的潜在 bug，确保即使所有消息在同一组且无组边界变化时，slave processor 也能被正确触发

**性能影响（Group=32）**：
- **低负载（≤3M TPS）**：P50 和 P99 基本持平或略有改善
- **中负载（4-5M TPS）**：P50 改善 3-4%，P99 改善 1-6%
- **高负载（6M TPS）**：P50 改善 9.1%，P99 略有上升（+15.3%）
- **极高负载（8M TPS）**：P50 改善 21.7%（11.5µs → 9µs），P99 改善 19.3%（161µs → 130µs）
- **整体评估**：Group=32 在大多数场景下表现更好，尤其是高负载的 P50 和 8M TPS 的 P99，建议保留