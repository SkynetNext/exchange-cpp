...
└───────────────────────────┼───────────────────────┼────────────────────────┘
                            ...
                            │                       │
┌───────────────────────────┴───────────────────────┴────────────────────────┐
│              Clearing & Settlement Service 集群（按用户分片，16台）              │
│                                                                              │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐  │
...
• 并行交互：所有撮合服务器同时与 Clearing & Settlement Service 交互（R1同步，R2异步）
• 用户分片：Clearing & Settlement Service 按 uid % 16 路由到对应 Shard，保证用户操作串行化
• 全局视图：每个 Clearing & Settlement Service Shard 维护所负责用户的所有币对状态
```

**数据流示例**：用户A同时在BTC/USDT和ETH/USDT下单
...
时间 T1: 用户A在撮合服务器1 (BTC/USDT) 下单
  ...
  │   └─ 同步调用 Clearing & Settlement Service Shard 4 (uid % 16 = 4)
  ...
  │
  └─ R2阶段：清算通知
      └─ 异步发送交易事件到 Clearing & Settlement Service Shard 4
...
时间 T2: 用户A在撮合服务器2 (ETH/USDT) 下单（并行）
  ...
  │   └─ 同步调用 Clearing & Settlement Service Shard 4 (uid % 16 = 4)  ← 同一Shard
...
Clearing & Settlement Service Shard 4 处理（串行化，按用户A）：
  │
...
| **并行交互** | 所有撮合服务器同时与 Clearing & Settlement Service 交互，互不阻塞 |
| **R1同步调用** | 调用 Clearing & Settlement Service 检查保证金，保证准确性，阻塞当前订单处理 |
| **R2异步通知** | 发送交易事件，不阻塞撮合流程，多个撮合服务器可同时发送 |
| **按用户分片** | Clearing & Settlement Service 按 `uid % 16` 分片，保证同一用户操作串行化 |
| **全局视图** | 每个Shard维护所负责用户的所有币对状态，支持跨币对风险计算 |
| **风险前置** | R1阶段拒绝超额订单，避免无效撮合，减少系统负载 |

**延迟分析**：
- **R1阶段**：Clearing & Settlement Service 调用 ~10-150μs（Aeron UDP，同机房 ~50-150μs，loopback ~10μs）
- **R2阶段**：异步通知，不阻塞撮合
- **优势**：准确性优先，避免超额下单

**业界采用**：Binance、Coinbase、OKX等大型交易所的主流方案
