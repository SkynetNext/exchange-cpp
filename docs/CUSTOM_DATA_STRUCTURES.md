...
| **无锁队列** | LMAX Disruptor | `disruptor-cpp` | 第三方库（子模块） | ✅ 已集成 | 无锁环形缓冲区，线程间通信 |
| **全局分配器** | - | `mimalloc` | 第三方库（子模块） | ✅ 已集成 | 高性能内存分配器，减少 50%+ 内存分配开销 |
| **压缩算法** | LZ4 Java (`lz4-java`) | `lz4` (C库) | 第三方库（子模块） | ✅ 已集成 | 快速压缩/解压缩，必需依赖（1:1 与 Java 版本） |
