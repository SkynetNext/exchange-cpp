...
| **Concurrent Queue** | `LinkedBlockingQueue` | `moodycamel::ConcurrentQueue` | Third-party library (submodule, header-only) | Lock-free concurrent queue, replaces `std::queue + std::mutex` in `SharedPool`. **Note**: `moodycamel::ConcurrentQueue` is unbounded and does not support capacity limits. Java version uses bounded `LinkedBlockingQueue` where `offer()` returns `false` when full (chains are discarded). C++ version uses unbounded queue for optimal performance. **Performance Comparison**: In `TestLatencyExchange`, `tbb::concurrent_bounded_queue` (poolMaxSize=64) shows performance cliff at 1.790 MT/s (latency jumps from 0.66µs to 2.15s, throughput drops 4.4x), while `moodycamel::ConcurrentQueue` maintains stable sub-microsecond latency. See `docs/PERFORMANCE_BENCHMARK_COMPARISON.md` for details. `poolMaxSize_` is kept for API compatibility but not enforced. |
