...
void OrderBookBaseTest::SetUp() {
    ...
    auto cmd1 =
        OrderCommand::NewOrder(OrderType::GTC, 1L, UID_1, 81600L, 0L, 100L, OrderAction::ASK);
    ProcessAndValidate(cmd1, CommandResultCode::SUCCESS);

    auto cmd2 =
        OrderCommand::NewOrder(OrderType::GTC, 2L, UID_1, 81599L, 0L, 50L, OrderAction::ASK);
    ProcessAndValidate(cmd2, CommandResultCode::SUCCESS);

    auto cmd3 =
        OrderCommand::NewOrder(OrderType::GTC, 3L, UID_1, 81599L, 0L, 25L, OrderAction::ASK);
    ProcessAndValidate(cmd3, CommandResultCode::SUCCESS);

    auto cmd8 =
        OrderCommand::NewOrder(OrderType::GTC, 8L, UID_1, 201000L, 0L, 28L, OrderAction::ASK);
    ProcessAndValidate(cmd8, CommandResultCode::SUCCESS);

    auto cmd9 =
        OrderCommand::NewOrder(OrderType::GTC, 9L, UID_1, 201000L, 0L, 32L, OrderAction::ASK);
...
void OrderBookBaseTest::TearDown() {
    ClearOrderBook();
}
...
void OrderBookBaseTest::ClearOrderBook() {
    ...
    // Match all asks
    int64_t askSum = std::accumulate(
        snapshot->askVolumes.begin(), snapshot->askVolumes.begin() + snapshot->askSize, 0LL);
    auto cmdAsk = OrderCommand::NewOrder(
        OrderType::IOC, 100000000000L, -1, MAX_PRICE, MAX_PRICE, askSum, OrderAction::BID);
    IOrderBook::ProcessCommand(orderBook_.get(), &cmdAsk);

    orderBook_->ValidateInternalState();

    // Match all bids
    int64_t bidSum = std::accumulate(
        snapshot->bidVolumes.begin(), snapshot->bidVolumes.begin() + snapshot->bidSize, 0LL);
    auto cmdBid =
...
void OrderBookBaseTest::TestShouldAddGtcOrders() {
    ...
    auto cmd94 = OrderCommand::NewOrder(
        OrderType::GTC, 94, UID_1, 81594, MAX_PRICE, 9'000'000'000L, OrderAction::BID);
    ...
    auto cmd95 = OrderCommand::NewOrder(
        OrderType::GTC, 95, UID_1, 130000, 0, 13'000'000'000L, OrderAction::ASK);
...
void OrderBookBaseTest::TestShouldMoveOrderExistingBucket() {
    ...
    auto expected =
        expectedState_->SetBidVolume(1, 41).IncrementBidOrdersNum(1).RemoveBid(2).Build();
    ASSERT_EQ(*expected, *snapshot);
...
void OrderBookBaseTest::TestShouldMatchIocOrderFullLiquidity() {
    auto cmd = OrderCommand::NewOrder(
        OrderType::IOC, 123, UID_2, MAX_PRICE, MAX_PRICE, 175, OrderAction::BID);
    ProcessAndValidate(cmd, CommandResultCode::SUCCESS);
...
void OrderBookBaseTest::TestShouldMatchIocOrderWithRejection() {
    auto cmd = OrderCommand::NewOrder(
        OrderType::IOC, 123, UID_2, MAX_PRICE, MAX_PRICE + 1, 270, OrderAction::BID);
    ProcessAndValidate(cmd, CommandResultCode::SUCCESS);
...
void OrderBookBaseTest::TestShouldRejectFokBidOrderOutOfBudget() {
    ...
    auto cmd = OrderCommand::NewOrder(
        OrderType::FOK_BUDGET, 123L, UID_2, buyBudget, buyBudget, size, OrderAction::BID);
    ProcessAndValidate(cmd, CommandResultCode::SUCCESS);
...
void OrderBookBaseTest::TestShouldRejectFokAskOrderBelowExpectation() {
    ...
    auto cmd = OrderCommand::NewOrder(OrderType::FOK_BUDGET,
                                      123L,
                                      UID_2,
                                      sellExpectation,
                                      sellExpectation,
...
void OrderBookBaseTest::TestShouldMatchFokAskOrderExactExpectation() {
    ...
    auto snapshot = orderBook_->GetL2MarketDataSnapshot(10);
    auto expected =
        expectedState_->RemoveBid(0).SetBidVolume(0, 1).DecrementBidOrdersNum(0).Build();
    ASSERT_EQ(*expected, *snapshot);
...
void OrderBookBaseTest::TestShouldFullyMatchMarketableGtcOrderWithAllLiquidity() {
    auto cmd = OrderCommand::NewOrder(
        OrderType::GTC, 123, UID_2, 220000, MAX_PRICE, 1000, OrderAction::BID);
    ProcessAndValidate(cmd, CommandResultCode::SUCCESS);
...
void OrderBookBaseTest::PrintOrderBookState(const std::string& label,
    ...
    LOG_INFO(
        ...
        "{}\nImplementation: {}",
        label,
        orderBook->GetStateHash(),
        orderBook->GetOrdersNum(OrderAction::ASK),
        orderBook->GetOrdersNum(OrderAction::BID),
        (orderBook->GetImplementationType() == exchange::core::orderbook::OrderBookImplType::DIRECT
             ? "DirectImpl"
             : "NaiveImpl"));

    // Print diagrams using base class interface
    LOG_INFO("\n--- Ask Buckets ---\n{}", orderBook->PrintAskBucketsDiagram());
    LOG_INFO("\n--- Bid Buckets ---\n{}", orderBook->PrintBidBucketsDiagram());
}

void OrderBookBaseTest::PrintOrderBookComparison(exchange::core::orderbook::IOrderBook* orderBook1,
