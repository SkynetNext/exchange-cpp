...
void OrderBookBaseTest::SetUp() {
  ...
  auto initialL2 = std::make_unique<L2MarketData>(askPrices, askVolumes, askOrders, bidPrices,
                                                  bidVolumes, bidOrders);
...
void OrderBookBaseTest::ClearOrderBook() {
  ...
  // Match all bids
  int64_t bidSum = std::accumulate(snapshot->bidVolumes.begin(),
...
void OrderBookBaseTest::PrintOrderBookState(const std::string& label,
  ...
  LOG_INFO(
    "\n=== {} ===\nState Hash: {}\nAsk orders: {}\nBid orders: "
    "{}\nImplementation: {}",
    label, orderBook->GetStateHash(), orderBook->GetOrdersNum(OrderAction::ASK),
