...
public:
  ...
  void ClientPlacesOrderWithReservePrice(
      int64_t clientId, const std::string &side, int64_t orderId, int64_t price,
      int64_t size, const std::string &orderType,
      const exchange::core::common::CoreSymbolSpecification &symbol,
  ...
  // to {word}
  void ClientCouldNotPlaceOrder(
      int64_t clientId, const std::string &side, int64_t orderId, int64_t price,
      int64_t size, const std::string &orderType,
      const exchange::core::common::CoreSymbolSpecification &symbol,
      int64_t reservePrice, const std::string &resultCode);

  // Then: The order {long} is partially matched. LastPx: {long}, LastQty:
  // {long}
  void OrderIsPartiallyMatched(int64_t orderId, int64_t lastPx,
                               int64_t lastQty);
  ...
  void OrderIsFullyMatchedWithBidderHoldPrice(int64_t orderId, int64_t lastPx,
                                              int64_t lastQty,
  ...
  void ClientMovesOrderPrice(int64_t clientId, int64_t newPrice,
                             int64_t orderId);

  // When: A client {user} could not move a price to {long} of the order {long}
  // due to {word}
  void ClientCouldNotMoveOrderPrice(int64_t clientId, int64_t newPrice,
                                    int64_t orderId,
  ...
  void AddBalanceToClient(int64_t amount, const std::string &currency,
                          int64_t clientId);
...
private:
  ...
  void TheOrderIsMatched(int64_t orderId, int64_t lastPx, int64_t lastQty,
                         bool completed, int64_t *bidderHoldPrice);

  void
  MoveOrder(int64_t clientId, int64_t newPrice, int64_t orderId,
            exchange::core::common::cmd::CommandResultCode expectedResultCode);

  void CheckField(const std::map<std::string, int> &fieldNameByIndex,
                  const std::vector<std::string> &record,
                  const std::string &field, int64_t expected);

  std::map<int64_t, exchange::core::common::Order> FetchIndexedOrders(
