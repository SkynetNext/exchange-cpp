...
void OrderStepdefs::ClientCouldNotPlaceOrder(
  ...
  if (resultCodeStr == "SUCCESS") {
    ...
    resultCode = exchange::core::common::cmd::CommandResultCode::RISK_INVALID_RESERVE_BID_PRICE;
  } else {
...
void OrderStepdefs::OrderIsPartiallyMatched(int64_t orderId, int64_t lastPx, int64_t lastQty) {
  TheOrderIsMatched(orderId, lastPx, lastQty, false, nullptr);
}

void OrderStepdefs::OrderIsFullyMatched(int64_t orderId, int64_t lastPx, int64_t lastQty) {
  TheOrderIsMatched(orderId, lastPx, lastQty, true, nullptr);
}

void OrderStepdefs::OrderIsFullyMatchedWithBidderHoldPrice(int64_t orderId,
                                                           int64_t lastPx,
                                                           int64_t lastQty,
...
void OrderStepdefs::ClientMovesOrderPrice(int64_t clientId, int64_t newPrice, int64_t orderId) {
  MoveOrder(clientId, newPrice, orderId, exchange::core::common::cmd::CommandResultCode::SUCCESS);
}

void OrderStepdefs::ClientCouldNotMoveOrderPrice(int64_t clientId,
                                                 int64_t newPrice,
                                                 int64_t orderId,
                                                 const std::string& resultCodeStr) {
  exchange::core::common::cmd::CommandResultCode resultCode;
  if (resultCodeStr == "SUCCESS") {
    ...
    resultCode =
      exchange::core::common::cmd::CommandResultCode::MATCHING_MOVE_FAILED_PRICE_OVER_RISK_LIMIT;
  } else {
    throw std::invalid_argument("Unknown result code: " + resultCodeStr);
  }
  ...
  for (const auto& orderEntry : orderEntries) {
    ...
    if (reservePriceIt != orderEntry.end()) {
      ...
      if (order.reserveBidPrice != expectedReservePrice) {
        throw std::runtime_error("Unexpected value for reservePrice: expected "
    ...
    if (sideIt != orderEntry.end()) {
      ...
      if (order.action != expectedAction) {
        throw std::runtime_error("Unexpected action: expected " + sideIt->second);
      }
    }
  }
}
...
void OrderStepdefs::ClientHasNoActiveOrders(int64_t clientId) {
  ...
  auto orders = FetchIndexedOrders(*profile);
  if (orders.size() != 0) {
    throw std::runtime_error("Expected no active orders, but got " + std::to_string(orders.size()));
  }
}
...
void OrderStepdefs::AddBalanceToClient(int64_t amount,
  ...
  auto adjustCmd = std::make_unique<exchange::core::common::api::ApiAdjustUserBalance>(
    clientId, currencyCode, amount, 2193842938742L);

  container_->SubmitCommandSync(std::move(adjustCmd),
                                exchange::core::common::cmd::CommandResultCode::SUCCESS);
}

void OrderStepdefs::ClientCancelsOrder(int64_t clientId, int64_t size, int64_t orderId) {
  ...
  container_->SubmitCommandSync(
    std::move(cancelCmd), [this, clientId, orderId, size,
      ...
      if (cmd.command != exchange::core::common::cmd::OrderCommandType::CANCEL_ORDER) {
      ...
      if (evt.size != size) {
        throw std::runtime_error("Unexpected size: expected " + std::to_string(size) + ", got "
...
void OrderStepdefs::AClientPassAnOrder(
  ...
  auto order = std::make_unique<exchange::core::common::api::ApiPlaceOrder>(
    price, size, orderId, action, type, clientId, symbol.symbolId, 0, reservePrice);
  ...
  auto future = container_->GetApi()->SubmitCommandAsyncFullResponse(order.release());
  ...
  if (cmd.resultCode != expectedResultCode) {
    throw std::runtime_error("Unexpected resultCode: expected "
  ...
  if (cmd.reserveBidPrice != reservePrice) {
    throw std::runtime_error("Unexpected reserveBidPrice: expected " + std::to_string(reservePrice)
                             + ", got " + std::to_string(cmd.reserveBidPrice));
  }

  matcherEvents_ = cmd.ExtractEvents();
}

void OrderStepdefs::TheOrderIsMatched(int64_t orderId,
                                      int64_t lastPx,
                                      int64_t lastQty,
                                      bool completed,
                                      int64_t* bidderHoldPrice) {
...
void OrderStepdefs::MoveOrder(int64_t clientId,
                              int64_t newPrice,
                              int64_t orderId,
                              exchange::core::common::cmd::CommandResultCode expectedResultCode) {
  auto initialOrderIt = orders_.find(orderId);
  ...
  container_->SubmitCommandSync(std::move(moveCmd),
...
std::map<int64_t, exchange::core::common::Order> OrderStepdefs::FetchIndexedOrders(
