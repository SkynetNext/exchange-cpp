  ...
  while (!activeChunks.empty()) {
    ...
    while (missCounter < 3 && !activeChunks.empty()) {
      ...
      for (size_t i = 0; i < activeChunks.size(); i++) {
        size_t chunkWeight = activeChunks[i].size - activeChunks[i].currentIndex;
      ...
      if (selectedChunk.currentIndex < selectedChunk.size) {
        ...
        mergedResult.push_back(std::move((*selectedChunk.data)[selectedChunk.currentIndex]));
    ...
    activeChunks.erase(
      std::remove_if(activeChunks.begin(), activeChunks.end(),
                     [](const ChunkInfo& chunk) { return chunk.currentIndex >= chunk.size; }),
      activeChunks.end());
  }
