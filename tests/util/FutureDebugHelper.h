 ...
 */
template <typename T>
T GetFutureWithDebug(std::future<T>&& future,
                     const char* file,
                     int line,
                     const char* function,
                     const char* context = "") {
...
/**
 * Helper function to get future value with detailed error reporting (reference version, for
 * reusable futures)
 */
template <typename T>
T GetFutureWithDebugRef(std::future<T>& future,
                        const char* file,
                        int line,
                        const char* function,
                        const char* context = "") {
...
#define GET_FUTURE_DEBUG(future, context)                                                \
  exchange::core::tests::util::GetFutureWithDebug(std::move(future), __FILE__, __LINE__, \
