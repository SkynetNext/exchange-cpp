...
L2MarketDataHelper::L2MarketDataHelper(const exchange::core::common::L2MarketData& l2) {
...
std::unique_ptr<exchange::core::common::L2MarketData> L2MarketDataHelper::Build() const {
...
L2MarketDataHelper& L2MarketDataHelper::SetAskVolume(int pos, int64_t askVolume) {
  askVolumes_[pos] = askVolume;
  return *this;
}

L2MarketDataHelper& L2MarketDataHelper::SetBidVolume(int pos, int64_t bidVolume) {
  bidVolumes_[pos] = bidVolume;
  return *this;
}

L2MarketDataHelper& L2MarketDataHelper::DecrementAskVolume(int pos, int64_t askVolumeDiff) {
  askVolumes_[pos] -= askVolumeDiff;
  return *this;
}

L2MarketDataHelper& L2MarketDataHelper::DecrementBidVolume(int pos, int64_t bidVolumeDiff) {
...
L2MarketDataHelper& L2MarketDataHelper::InsertAsk(int pos, int64_t price, int64_t volume) {
...
L2MarketDataHelper& L2MarketDataHelper::InsertBid(int pos, int64_t price, int64_t volume) {
...
std::string
L2MarketDataHelper::DumpOrderBook(const exchange::core::common::L2MarketData& l2MarketData) const {
  ...
  std::ostringstream s;
  s << "Order book:\n";
  s << "." << std::string(priceWidth - 2, '-') << "ASKS" << std::string(volWidth - 1, '-') << ".\n";
  for (int i = askSize - 1; i >= 0; i--) {
    s << "|" << std::setw(priceWidth) << askPrices[i] << "|" << std::setw(volWidth) << askVolumes[i]
      << "|" << std::setw(ordWidth) << askOrders[i] << "|\n";
  }
  s << "|" << std::string(priceWidth, '-') << "+" << std::string(volWidth, '-') << "|\n";
  for (int i = 0; i < bidSize; i++) {
    s << "|" << std::setw(priceWidth) << bidPrices[i] << "|" << std::setw(volWidth) << bidVolumes[i]
      << "|" << std::setw(ordWidth) << bidOrders[i] << "|\n";
  }
  s << "'" << std::string(priceWidth - 2, '-') << "BIDS" << std::string(volWidth - 1, '-') << "'\n";
  return s.str();
}

int L2MarketDataHelper::MaxWidth(int minWidth,
